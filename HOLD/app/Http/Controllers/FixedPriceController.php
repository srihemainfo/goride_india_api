<?php

namespace App\Http\Controllers;

use App\Models\Price;
use Illuminate\Http\Request;
use Illuminate\Validation\Rule;
use Illuminate\Support\Facades\{Validator, DB};
use Yajra\DataTables\Facades\DataTables;
use App\Services\Permissions\PermissionHelperService;

class FixedPriceController extends Controller
{
    private $module = 'FIXED_PRICE_MODULE';
    private $permission;

    public function __construct()
    {
        $this->permission = new PermissionHelperService;
    }

    public function index(Request $request)
    {
        $this->permission->check_privilege($this->module, self::ACTION_TYPE['index']);

        //UI permissions array destructured
        [
            'CREATE' => $IS_CREATABLE,
            'UPDATE' => $IS_UPDATABLE,
            'DELETE' => $IS_DELETABLE
        ] = $this->permission->ui_permissions($this->module);

        $list_places = DB::table('place')->select('id', 'place')
            ->where('status', '=', 'Active')
            ->get();

        $list_areas = DB::table('area')->select('id', 'area')
            ->where('status', '=', 'Active')
            ->get();

        if ($request->ajax()) {
            $data = DB::table('admin_form')
                ->select('*')
                ->where([
                    ['id', '!=', ''], ['zone', '=', '0']
                ])
                ->where(function ($query) {
                    $query->where('temp', '!=', 'yes')
                        ->orWhereNull('temp');
                });

            return $this->Datatable($data, $request);
        }

        return view('prices.fixed.index', compact('list_places', 'list_areas', 'IS_CREATABLE'));
    }

    public function store(Request $request)
    {
        if(isset($request->price_id)){
            $this->permission->check_privilege($this->module, self::ACTION_TYPE['update']);
        } else {
            $this->permission->check_privilege($this->module, self::ACTION_TYPE['store']);
        }

        $validator = Validator::make(
            $request->all(),
            [
                'place_from' => ['required'],
                'area_from' => ['required'],
                'place_to' => ['required'],
                'area_to' => ['required', Rule::unique('admin_form')
                    ->where('area_from', $request->area_from)
                    ->where('area_to', $request->area_to)
                    ->ignore($request->price_id)],
                'sedan' => ['nullable'],
                'minivan' => ['nullable'],
                'seater7' => ['nullable'],
                'seater8' => ['nullable'],
                'executive' => ['nullable'],
                'mpv_executive' => ['nullable'],
                'sharedride' => ['nullable'],
                'seater32' => ['nullable'],
                'seater44' => ['nullable'],
                'sixteenseater' => ['nullable'],
                'twotwoseater' => ['nullable'],
                'seater55' => ['nullable']
            ],
            [
                'area_to.unique' => 'Given from and to areas combination already exist.'
            ]
        );

        if ($validator->fails()) {
            return response()->json(['status' => 400, 'errors' => $validator->errors()]);
        } else {
            // dd($request->minivan);
            $data = Price::updateOrCreate(
                ['id' => $request->price_id],
                [
                    'place_from' => $request->place_from,
                    'area_from' => $request->area_from,
                    'place_to' => $request->place_to,
                    'area_to' => $request->area_to,
                    'sedan' => sanitize_amount_input($request->sedan),
                    'minivan' => sanitize_amount_input($request->minivan),
                    'seater7' => sanitize_amount_input($request->seater7),
                    'seater8' => sanitize_amount_input($request->seater8),
                    'executive' => sanitize_amount_input($request->executive),
                    'mpv_executive' => sanitize_amount_input($request->mpv_executive),
                    'sharedride' => sanitize_amount_input($request->sharedride),
                    'seater32' => sanitize_amount_input($request->seater32),
                    'seater44' => sanitize_amount_input($request->seater44),
                    'sixteenseater' => sanitize_amount_input($request->sixteenseater),
                    'twotwoseater' => sanitize_amount_input($request->twotwoseater),
                    'seater55' => sanitize_amount_input($request->seater55)
                ]
            );

            return response()->json($data->id ? ['status' => 200, 'data' => $data, 'errors' => NULL] : ['status' => 400, 'data' => NULL, 'errors' => NULL]);
        }
    }

    public function edit($id)
    {
        $this->permission->check_privilege($this->module, self::ACTION_TYPE['edit']);
        $price = Price::find($id);
        return response()->json($price ? ['status' => 200, 'data' => $price] : ['status' => 400, 'data' => NULL]);
    }

    public function destroy($id)
    {
        $this->permission->check_privilege($this->module, self::ACTION_TYPE['destroy']);
        $data = DB::table('admin_form')->where('id', '=', $id)->delete();
        return response()->json($data ? ['status' => 200, 'isDeleted' => true] : ['status' => 400, 'isDeleted' => false]);
    }

    public function FixedPriceUpdate(Request $request)
    {
        $this->permission->check_privilege($this->module, self::ACTION_TYPE['update']);
        $data = Price::updateOrCreate(
            ['id' => $request->price_id],
            [
                    'place_from' => $request->place_from,
                    'area_from' => $request->area_from,
                    'place_to' => $request->place_to,
                    'area_to' => $request->area_to,
                    'sedan' => sanitize_amount_input($request->sedan),
                    'minivan' => sanitize_amount_input($request->minivan),
                    'seater7' => sanitize_amount_input($request->seater7),
                    'seater8' => sanitize_amount_input($request->seater8),
                    'executive' => sanitize_amount_input($request->executive),
                    'mpv_executive' => sanitize_amount_input($request->mpv_executive),
                    'sharedride' => sanitize_amount_input($request->sharedride),
                    'seater32' => sanitize_amount_input($request->seater32),
                    'seater44' => sanitize_amount_input($request->seater44),
                    'sixteenseater' => sanitize_amount_input($request->sixteenseater),
                    'twotwoseater' => sanitize_amount_input($request->twotwoseater),
                    'seater55' => sanitize_amount_input($request->seater55)
            ]
        );
        return  response()->json($data->id ? ['status' => 200, 'isUpdated' => true] : ['status' => 400, 'isUpdated' => false]);
    }

    public function GetAreaForPlace($place_id)
    {
        $list_areas = DB::table('area')->select('area')
            ->where([
                ['place_id', '=', $place_id], ['status', '=', 'Active']
            ])
            ->orderBy('area', 'ASC')
            ->get();

        return response()->json(count($list_areas) > 0 ? ['status' => 200, 'data' => $list_areas] : ['status' => 404, 'data' => NULL]);
    }

    private function Datatable($data, $request)
    {
        //UI permissions array destructured
        [
            'CREATE' => $IS_CREATABLE,
            'UPDATE' => $IS_UPDATABLE,
            'DELETE' => $IS_DELETABLE
        ] = $this->permission->ui_permissions($this->module);

        return DataTables::of($data)
            ->addIndexColumn()
            ->filter(function ($query) use ($request) {
                if ($request->has('selected_area_from')) {
                    $query->where('area_from', '=', $request->get('selected_area_from'));
                }
                if ($request->has('selected_area_to')) {
                    $query->where('area_to', '=', $request->get('selected_area_to'));
                }
            })
            ->addColumn('sedan', function ($row) use ($IS_UPDATABLE) {
                
                if ($IS_UPDATABLE) {
                    $sedan = "<div class=\"input-group\">
                                <input name=\"sedan\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->sedan\">
                        </div>";
                } else {
                        $sedan = "<div style=\"text-align: right;\">$row->sedan</div>";
                }
                return $sedan;
            })
            ->addColumn('minivan', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $minivan = "<div class=\"input-group\">
                                <input name=\"minivan\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->minivan\">
                        </div>";
                } else {
                        $minivan = "<div style=\"text-align: right;\">$row->minivan</div>";
                }
                return $minivan;
            })
            ->addColumn('seater7', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $seater7 = "<div class=\"input-group\">
                                <input name=\"seater7\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->seater7\">
                        </div>";
                } else {
                    $seater7 = "<div style=\"text-align: right;\">$row->seater7</div>";
                }
                return $seater7;
            })
            ->addColumn('seater8', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $seater8 = "<div class=\"input-group\">
                                <input name=\"seater8\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->seater8\">
                        </div>";
                } else {
                    $seater8 = "<div style=\"text-align: right;\">$row->seater8</div>";
                }
                return $seater8;
            })
            ->addColumn('executive', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $executive = "<div class=\"input-group\">
                                <input name=\"executive\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->executive\">
                        </div>";
                } else {
                    $executive = "<div style=\"text-align: right;\">$row->executive</div>";
                }
                
                return $executive;
            })
            ->addColumn('mpv_executive', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $mpv_executive = "<div class=\"input-group\">
                                <input name=\"mpv_executive\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->mpv_executive\">
                        </div>";
                } else {
                    $mpv_executive = "<div style=\"text-align: right;\">$row->mpv_executive</div>";
                }
                return $mpv_executive;
            })
            ->addColumn('sharedride', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $sharedride = "<div class=\"input-group\">
                                <input name=\"sharedride\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->sharedride\">
                        </div>";
                } else {
                    $sharedride = "<div style=\"text-align: right;\">$row->sharedride</div>";
                }
                return $sharedride;
            })
            ->addColumn('sixteenseater', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $sixteenseater = "<div class=\"input-group\">
                                <input name=\"sixteenseater\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->sixteenseater\">
                        </div>";
                } else {
                    $sixteenseater = "<div style=\"text-align: right;\">$row->sixteenseater</div>";
                }
                return $sixteenseater;
            })
            ->addColumn('twotwoseater', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $twotwoseater = "<div class=\"input-group\">
                                <input name=\"twotwoseater\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->twotwoseater\">
                        </div>";
                } else {
                    $twotwoseater = "<div style=\"text-align: right;\">$row->twotwoseater</div>";
                }
                return $twotwoseater;
            })
            ->addColumn('seater32', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $seater32 = "<div class=\"input-group\">
                                <input name=\"seater32\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->seater32\">
                        </div>";
                } else {
                    $seater32 = "<div style=\"text-align: right;\">$row->seater32</div>";
                }
                return $seater32;
            })
            ->addColumn('seater44', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $seater44 = "<div class=\"input-group\">
                                <input name=\"seater44\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->seater44\">
                        </div>";
                } else {
                    $seater44 = "<div style=\"text-align: right;\">$row->seater44</div>";
                }
                return $seater44;
            })
            ->addColumn('seater55', function ($row) use ($IS_UPDATABLE) {
                if ($IS_UPDATABLE) {
                    $seater55 = "<div class=\"input-group\">
                                <input name=\"seater55\" type=\"text\" class=\"form-control form-control-30 p_extra\" placeholder=\"\"  style=\"text-align: right;\" value=\"$row->seater55\">
                        </div>";
                } else {
                    $seater55 = "<div style=\"text-align: right;\">$row->seater55</div>";
                }
                return $seater55;
            })
            
            ->addColumn('action', function ($row) use ($IS_UPDATABLE, $IS_DELETABLE) {
                $btn = '';

                if ($IS_UPDATABLE) {
                    $btn = '<a href="javascript:void(0)" data-id="' . $row->id . '" title="Update Item" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-success updatePrice"><i class="fa fa-check"></i></a>';
                }
                if ($IS_UPDATABLE) {
                    $btn = $btn . '<a href="javascript:void(0)" data-id="' . $row->id . '" title="Edit Item" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-primary editPrice"><i class="fa fa-edit"></i></a>';
                }
                if ($IS_DELETABLE) {
                    $btn = $btn . '<a href="javascript:void(0)" data-id="' . $row->id . '" title="Delete Item" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-danger deletePrice"><i class="fa fa-trash"></i></a';
                }
                return $btn;
            })
            ->rawColumns(['sedan', 'minivan', 'seater7', 'seater8', 'executive', 'mpv_executive', 'sharedride', 'sixteenseater','twotwoseater', 'seater32', 'seater44', 'seater55',  'action'])
            ->make(true);
    }
}
