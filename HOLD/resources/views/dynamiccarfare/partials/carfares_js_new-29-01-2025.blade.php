<script>
            ...
            success: function (response) {
                ...
                if (response.data) {
                    ...
                    $(document).on('click', '#addButton', function () {
                        ...
                        if (currentvalue != 0) {
                            removeButton = `<td><button class='removeButton btn btn-danger'>RemoveRow</button> </td>`;
    ...
    const showMakeOver = () => {
        try {
            ...
            $('#header_container th:nth-child(1)').text('START KM/M');
            $('#header_container th:nth-child(2)').text('END KM/M');



            $('#table-body tr').each(function (index, v) {


                let id = index + 1;

                // console.log($(`#start_${id}`).val());
                // console.log($(`#end_${id}`).val());

                $(v).find('.addExtraValue').remove();
                ...
                //     if (index !== 0 && index !== $('#table-body tr').length - 1) {

                //        $(row).find('td:last').append('<button class="removeButton btn btn-danger">RemoveRow</button>');
                //    }
            ...
            $(`#table-body tr:eq(${$('#table-body tr').length - 1}) td:last`).append('<a onclick="$(\'#end\').val(parseInt($(\'#start\').val()) + 10);$(\'#addButton\').trigger(\'click\');" class="btn btn-primary addExtraValue">Add</a>');

            $(`#table-body tr:eq(${$('#table-body tr').length - 1}) td:last`).find('.removeButton').remove();
