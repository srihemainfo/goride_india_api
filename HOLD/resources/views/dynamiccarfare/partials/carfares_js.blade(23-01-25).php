...
//                     if (startValue > endvalue) {
//                         swalalerterror('Start fare cannot be higher than end');
//                         return;
//                     } else if (startValue == endvalue) {
//                         swalalerterror('Start and end cannot have the same value');
//                         return;
//                     }
...
//                     // Create new row with "Remove" button
//                     var removeButton = `<th><button class='removeButton btn btn-danger'>Remove Row</button></th>`;
...
//                         <tr id='current_${currentvalue}'>
//                             <td> 
//                                 <span id="current_text_start_${currentvalue}">${startValue}</span> 
//                                 <input type='hidden' data-start='${startValue}' class='form-control' id="start_${currentvalue}" name='start[]' value="${startValue}">
//                             </td>
//                             <td>
//                                 <span id="current_text_end_${currentvalue}" style='display:none;'>${endvalue}</span>
//                                 <input id="end_${currentvalue}" class='form-control' type='text' oninput='validateInputInteger(this)' name='end[]' value="${endvalue}">
//                             </td>`;
...
//                         if (carNames.hasOwnProperty(key)) {
//                             var carName = carNames[key];
//                             appendtbody += `<td><input type='text' class='editable-input' oninput='validateInputInteger(this)' name='${carName}[]' class='form-control' value='' data-column='${carName}'></td>`;
//                         }
//                     }
...
//                     // Disable the current "Add" button and make it read-only for the added row
//                     $(this).text('Remove').attr('id', 'removeButton').attr('disabled', 'disabled');
//                     $('#start, #end').attr('disabled', true);

//                     // Increment the current row index
//                     currentvalue++;

//                     // Add a new row for further entry after this
//                     var newRow = `
//                         <tr id="current_${currentvalue}">
//                             <td><span id="current_text_start_${currentvalue}"></span> <input type="hidden" id="start_${currentvalue}" name="start[]" class="form-control"></td>
//                             <td><span id="current_text_end_${currentvalue}"></span> <input type="text" id="end_${currentvalue}" name="end[]" class="form-control"></td>
//                             <td><button id="addButton" class="btn btn-success">Add New Range</button></td>
//                         </tr>
//                     `;
//                     $("#table-body").append(newRow);
//                 });

//                 // Remove button logic for removing a specific row
//                 $(document).on('click', '.removeButton', function () {
//                     var rowToRemove = $(this).closest('tr');
//                     rowToRemove.remove();

//                     // Re-enable the "Add New Range" button in the new last row
//                     var lastRow = $('#table-body tr').last();
//                     lastRow.find('button').text('Add New Range').attr('id', 'addButton').removeAttr('disabled');
//                 });
//             }
//         },
//         error: function (error) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.data) {
                ...
                // Create first row with default value and + button
                var initialRow = `
                    <tr id="current_0">
                        <td>
                            <span id="current_text_start_0">0</span>
                            <input type="hidden" id="start_0" name="start[]" class="form-control" value="0">
                        </td>
                        <td>
                            <input id="end_0" class="form-control" type="text" value="0" oninput="validateInputInteger(this)" name="end[]">
                        </td>
                        <td><button id="addButton" class="btn btn-success">+</button></td>
                    </tr>
                `;
                $("#table-body").append(initialRow);

                // Add button click handler to create new row
                $(document).on('click', '#addButton', function () {
                    var start, end;
                    ...
                    // Create new row with editable inputs and remove button
                    var newRow = '';
                    newRow += `
                        <tr id="current_${currentvalue}">
                            <td>
                                <span id="current_text_start_${currentvalue}">${startValue}</span>
                                <input type="hidden" id="start_${currentvalue}" name="start[]" class="form-control" value="${startValue}">
                            </td>
                            <td>
                                <span id="current_text_end_${currentvalue}" style="display:none;">${endvalue}</span>
                                <input id="end_${currentvalue}" class="form-control" type="text" value="${endvalue}" oninput="validateInputInteger(this)" name="end[]">
                            </td>
                    `;
                    ...
                    for (let key in carNames) {
                        if (carNames.hasOwnProperty(key)) {
                            var carName = carNames[key];
                            newRow += `<td><input type="text" class="editable-input form-control" name="${carName}[]" data-column="${carName}"></td>`;
                        }
                    }
                    ...
                    // Append new row after the initial one
                    $("#table-body").append(newRow);

                    // Disable the current "Add" button
                    $(this).attr('id', 'removeButton').text('-');

                    // Increment row value for the next row
                    currentvalue++;
                ...
                $(document).on('click', '.removeButton', function () {
                    ...
                    // Re-enable the "Add New Range" button in the new last row
                    var lastRow = $('#table-body tr').last();
                    lastRow.find('button').text('+').attr('id', 'addButton');
                });
            }
        },
        error: function (error) {
