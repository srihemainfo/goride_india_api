        ...
        success: function(response) {
            ...
            if (response.data) {
                ...
                $(document).on('click', '#addButton', function() {
                    ...
                    if (startValue >= endValue) {
                        swalalerterror('Start fare cannot be greater than or equal to end fare');
                        return;
                    }

                    // Remove button logic (changed to '-' symbol)
                    var removeButton = `<td><button class='removeButton btn btn-danger'>-</button></td>`;
                    ...
                    for (let key in carNames) {
                        if (carNames.hasOwnProperty(key)) {
                            var carName = carNames[key];
                            appendTbody += `<td><input type='text' class='editable-input form-control' name='${carName}[]' value='' oninput='validateInputInteger(this)'></td>`;
                        }
                    }
                ...
                $(document).on('click', '.removeButton', function() {
                    ...
                    if (previousRow.length) {
                        var previousStartValue = previousRow.find('input[name="start[]"]').val();
                        var nextTag = closestTr.next('tr');
                        nextTag.find('input[name="start[]"]').val(previousStartValue);
                        nextTag.find('span[id^="current_text_start_"]').text(previousStartValue);
                    }
