...
$(document).ready(function () {
    ...
    // Sync Outstation above km inputs
    ...
    $('#hourly_input, #km_input, #day_input').on('input', function () {
        ...
        function calc(normal_hr, normal_km, normal_amt, add_km, add_hr,
            ...
            let outstation_total = out_min_amt + ((km - out_above) * out_km);

            // ✅ Bata logic
            if (upto_km > 0) {
                if (km > upto_km) {
                    let balance = km - upto_km;
                    let extraDays = Math.ceil(balance / upto_km);
                    outstation_total += (extraDays * out_day_price);
                }
            } else {
                outstation_total += (out_day * out_day_price);
            }

            return [normal_total, outstation_total, upto_km];
        }
        ...
        let customF1 = applyMinimumLogic(km, hr, parseFloat($('#first_km').val()), parseFloat($('#first_hr').val()), parseFloat($('#first_amount').val()), parseFloat($('#first_additional_km').val()), parseFloat($('#first_additional_hr').val()));
        if (customF1 !== false) f1 = customF1;

        let customF2 = applyMinimumLogic(km, hr, parseFloat($('#second_km').val()), parseFloat($('#second_hr').val()), parseFloat($('#second_amount').val()), parseFloat($('#second_additional_km').val()), parseFloat($('#second_additional_hr').val()));
        if (customF2 !== false) f2 = customF2;

        let customF3 = applyMinimumLogic(km, hr, parseFloat($('#third_km').val()), parseFloat($('#third_hr').val()), parseFloat($('#third_amount').val()), parseFloat($('#third_additional_km').val()), parseFloat($('#third_additional_hr').val()));
        if (customF3 !== false) f3 = customF3;
        ...
        // Toggle inputs
        toggleFareAndInput(km, '#first_Outstation_above_km', '#first_total', '#first_outstation_total', u1);
        toggleFareAndInput(km, '#second_Outstation_above_km', '#second_total', '#second_outstation_total', u2);
        toggleFareAndInput(km, '#third_Outstation_above_km', '#third_total', '#third_outstation_total', u3);
    });

    // Toggle logic including upto_km check
    function toggleFareAndInput(km, aboveSelector, fareSelector, outstationSelector, uptoKm) {
        ...
        if (km >= aboveKm) {
            ...
            $('#day_input').closest('.input-group').removeClass('d-none'); // show otherwise

            // ✅ Apply upto_km hide/show
            // if (uptoKm > 0) {
            //     $('#day_input').closest('.input-group').addClass('d-none'); // hide if upto_km > 0
            // } else {
            //     $('#day_input').closest('.input-group').removeClass('d-none'); // show otherwise
            // }
