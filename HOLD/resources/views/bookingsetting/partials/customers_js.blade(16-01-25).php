...
$(document).ready(function () {
    const countries = [
        ...
        'Ghana', 'Gibraltar', 'Greece', 'Greenland', 'Grenada', 'Guadeloupe', 'Guam', 'Guatemala', 'Guernsey', 'Guinea', 'Guinea-Bissau', 'Guyana', 'Haiti', 'Heard Island and McDonald Islands', 'Honduras', 'Hong Kong', 'Hungary', 'Iceland', 'India', 'Indonesia', 'Iran', 'Iraq', 'Ireland', 'Isle of Man', 'Israel', 'Italy', 'Ivory Coast', 'Jamaica', 'Japan', 'Jersey', 'Jordan', 'Kazakhstan', 'Kenya', 'Kiribati', 'Kuwait', 'Kyrgyzstan', 'Laos', 'Latvia', 'Lebanon', 'Lesotho', 'Liberia', 'Libya', 'Liechtenstein', 'Lithuania', 'Luxembourg', 'Macau', 'Macedonia', 'Madagascar', 'Malawi', 'Malaysia', 'Maldives', 'Mali', 'Malta', 'Marshall Islands', 'Martinique', 'Mauritania', 'New Caledonia', 'Mauritius', 'Mayotte', 'Mexico', 'Monaco', 'Mongolia', 'Montenegro', 'Montserrat', 'Morocco', 'Mozambique', 'Myanmar', 'Namibia', 'Nauru', 'Nepal', 'Netherlands', 'New Caledonia', 'New Zealand', 'Nicaragua', 'Niger', 'Nigeria', 'Niue', 'Norfolk Island', 'North Korea', 'Northern Mariana Islands', 'Norway', 'Oman', 'Pakistan', 'Palau', 'Palestinian Territory, Occupied', 'Panama', 'Papua New Guinea', 'Paraguay', 'Peru', 'Philippines', 'Pitcairn', 'Poland', 'Portugal', 'Puerto Rico', 'Qatar', 'Republic of Moldova', 'Republic of the Congo', 'Reunion', 'Romania', 'Russia', 'Rwanda', 'Saint Barthelemy', 'Saint Helena', 'Saint Kitts and Nevis', 'Saint Lucia', 'Saint Martin (French part)', 'Saint Pierre and Miquelon', 'Saint Vincent and the Grenadines', 'Samoa', 'San Marino', 'Sao Tome and Principe', 'Saudi Arabia', 'Senegal', 'Serbia', 'Seychelles', 'Sierra Leone', 'Singapore', 'Sint Maarten', 'Slovakia', 'Slovenia', 'Solomon Islands', 'Somalia', 'South Africa', 'South Georgia and the South Sandwich Islands', 'South Korea', 'Spain', 'Sri Lanka', 'Sudan', 'Suriname', 'Svalbard and Jan Mayen', 'Swaziland', 'Sweden', 'Switzerland', 'Syrian Arab Republic', 'Taiwan', 'Tajikistan', 'Tanzania', 'Thailand', 'Togo', 'Tokelau', 'Tonga', 'Trinidad and Tobago', 'Tunisia', 'Turkey', 'Turkmenistan', 'Turks and Caicos Islands', 'Tuvalu', 'Uganda', 'Ukraine', 'United Arab Emirates', 'United Kingdom', 'United States', 'United States Minor Outlying Islands', 'United States Virgin Islands', 'Uruguay', 'Uzbekistan', 'Vanuatu', 'Vatican City', 'Venezuela', 'Vietnam', 'Wallis and Futuna', 'Western Sahara', 'Yemen', 'Zambia', 'Zimbabwe'
    ];
    ...
    $('#country').on('blur', function () {
        let inputValue = $(this).val().trim(); // Use trim() to avoid whitespace issues

        // Check if the input is not empty and the dropdown is not active
        if (inputValue && !$('#dropdown').hasClass('active')) {
            // Convert the input value to lowercase for case-insensitive comparison
            const isValidCountry = countries.map(country => country.toLowerCase()).includes(inputValue.toLowerCase());
            if (!isValidCountry) {
                $(this).val(''); 
...
// timezone    
 $(document).ready(function () {       
const timezone = [
          'Asia/Kolkata',  'Africa/Abidjan', 'Africa/Accra', 'Africa/Addis_Ababa', 'Africa/Algiers', 'Africa/Asmara', 'Africa/Bamako', 'Africa/Bangui', 
 ...
 $('#timezone').on('input', function () {
    ...
    $(document).on('click', '.dropdown', function () {
        let selectedTimezone = $(this).text();
    ...
    $('#timezone').on('blur', function () {
        ...
        // Only validate if the dropdown is not open
        if (inputValue && !$('#dropdowntimezone').hasClass('active') && !timezone.includes(inputValue)) {
            $(this).val(''); 
            warningClick('Error', 'Please type a valid timezone from the list', 'danger');
        }
    });
...
// currency
 ...
 $('#currency').on('input', function () {
        let searchValue = $(this).val().toLowerCase();
        let filteredCurrencies = currency.filter(c => c.toLowerCase().includes(searchValue));

        $('#dropdowncurrency').empty();

        if (searchValue && filteredCurrencies.length) {
            $('#dropdowncurrency').addClass('active');
            filteredCurrencies.forEach(function (currencyItem) {
                $('#dropdowncurrency').append('<div class="dropdown-currency">' + currencyItem + '</div>');
            });
        } else {
    ...
    $('#currency').on('blur', function () {
        let inputValue = $(this).val();
        // Only validate if the dropdown is not open
        if (inputValue && !$('#dropdowncurrency').hasClass('active') && !currency.includes(inputValue)) {
            $(this).val(''); 
            warningClick('Error', 'Please select a valid currency from the list', 'danger');
        }
    });
    ...
    // Additional code for other dropdowns
