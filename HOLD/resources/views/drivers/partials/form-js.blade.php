<script>
    ...
    $('#addFileupload').click(function () {
        ...
        let select_file = $('#select_d_file').val('');
    ...
    $("#file_upload_btn").click(function (e) {
        ...
        var driver_n_id = $('#driver_n_id').val();

        formData.append('description', description);
        formData.append('driver_id', driver_n_id);
        formData.append('token', getCookie('d_token'));
        var fileInput = $('#select_d_file')[0].files[0];
        if (fileInput) {
            formData.append('select_file', fileInput);
        }
        if (description != '' && driver_n_id != '' && fileInput != '') {
            $.ajax({
                url: "{{env('API_URL')}}FileUpload",
                method: "POST",
                data: formData,
                dataType: "JSON",
                contentType: false,
                ...
                success: function (data) {
                    // $("#upload_d_form")[0].reset();
                    $('#description').val('');
                    $('#select_d_file').val('');
                    $('#select_d_file').replaceWith($('#select_d_file').clone(true));

                    if (data.isUploaded) {
                        $('#unique-model-2').modal('hide');
                        ResetErrors();
                        $('#upload_d_form').trigger("reset");
                        $('#form-modal').modal('hide');
                        $('#uploaded_documents_view').html('');

                        if (data.document_details != '') {
                            MakeDocumentsView(data.document_details, data.d_name);
                        }
                        ...
                        Swal.fire({
                            ...
                            if (window.location.href.includes("driver/create")) {
            ...
            warningClick('Error', "Fill the Description and Image Field", "danger");
        }
                    ...
                    success: function (response) {
                        ...
                        if (response.isDeleted) {
                            ...
                            if (response.document_details != '') {
                                ...
                                MakeDocumentsView(response.document_details, response.d_name);
    ...
    ///////////////////////////////////////////////////////////////
    function MakeDocumentsView(document_details, d_key) {
        ...
        $.each(document_details, function (key, item) {
            ...
            <td>\
                <a href="javascript:void(0);" onclick="viewImg(\'' + d_key + '\', \'' + item.file_path + '\')" id="document_view" class="mb-2 mr-2 btn-sm btn-transition btn" style="color: #266444; border-color:#266444;" title="View Document">\
                    <i class="fa fa-eye"></i>\
                </a>\
                <button type="button" class="mb-2 mr-2 btn-sm btn btn-outline-danger document_delete" data-id="' + item.id + '" title="Delete Document">\
    ...
    function viewImg(a, b) {
        console.log(a, b);
        let imageUrl = '/driver-documents/' + a + '/' + b;
        $('#img_doc').attr('src', imageUrl);
        $('#unique-model-3').modal('show');
    }
            ...
            if (val_validate) {
                ...
                $.ajax(settings).done(function (response) {
                    ...
                    if (response.status === 200) {
                        $('#driver_sub').hide();
                        setCookie('swal', response['message'], '1');
                        $('#driver_n_id').val(response.data)
                        $('#driver_id_file').val(response.data)
    ...
    function validateForm() {
        ...
        let vehicleInsurance = document.getElementById('vehicle_insurance');

        let vehicleInsuranceError = document.getElementById('vehicle_insurance_Error');

        if (!vehicleInsurance.value.trim()) {

            vehicleInsuranceError.textContent = "Vehicle Insurance is required.";

            isValid = false;

        } else {

            vehicleInsuranceError.textContent = "";

        }

        let vehicleLicence = document.getElementById('vehicle_license');

        let vehicleLicenceError = document.getElementById('vehicle_license_Error');

        if (!vehicleLicence.value.trim()) {

            vehicleLicenceError.textContent = "Vehicle Licence is required.";

            isValid = false;

        } else {

            vehicleLicenceError.textContent = "";
        ...
        let driverLicense = document.getElementById('driver_license_no');

        let driverLicenseError = document.getElementById('driver_license_no_Error');

        if (!driverLicense.value.trim()) {

            driverLicenseError.textContent = "Driver License is required.";

            isValid = false;

        } else {

            driverLicenseError.textContent = "";
    ...
    $('#driver_sub_up').on('click', function () {
        ...
        if (driverId != '') {
            $('#driver_sub_up').html(`<div class="spinner-border" role="status" style="width: 1rem !important; height: 1rem !important;">
        ...
        $.ajax(settings).done(function (response) {
            ...
            $('#driver_sub_up').html(`<i class="fa fa-save"></i>&nbsp; Update`);
            ...
            if (status == 400) {
                ...
                warningClick('Error', message, "danger");
    ...
    function driveredit(id) {
        ...
        $.ajax(settings).done(function (response) {
            // console.log(response['d_name'], 'heeeeeeee')
            if (response['status'] == 200) {

                let cleanedDialCode = String(dial_code_store).replace("+", "");
                let phoneNumber = String(response['driver'][0].phone);
                // Remove prefix if exists
                if (phoneNumber.startsWith(cleanedDialCode)) {
                    phoneNumber = phoneNumber.slice(cleanedDialCode.length);
                }

                console.log('dial_code_store val:', cleanedDialCode);
                console.log('phone number:', phoneNumber);
                console.log('sliced', phoneNumber);

                if (window.location.href.includes("/edit/")) {
                    MakeDocumentsView(response['document_details'], response['d_name']);
                }

                $('#uploadPreview').val(response['driver'][0].upload_photo)

                $('#driver_id').val(response['driver_id'])

                $('#driver_n_id').val(response['driver_id'])
               ...
               var imagePath = response['driver'][0].upload_photo;
                var image_id = response['driver'][0].id;
                
                var show_image = imagePath.startsWith('http') ? imagePath : ("{{ url('/') }}/" + imagePath);
                console.log("Image path:", show_image);
                
                if (imagePath && imagePath !== '') {
                    $('#uploadPreview123').attr('src', show_image);
                    $('#deleteImageBtn').attr('onclick', 'deleteImages(' + image_id + ')');
                    $('#deleteImageBtn_remove').addClass('d-flex');
                } else {
                    $('#uploadPreview123').attr('src', '');
                    $('#deleteImageBtn_remove').removeClass('d-flex');
                }


                // console.log('Jana', test_imae);

                // console.log(response['driver'][0].upload_photo);

                $('#upload_photo').html(test_imae)
    ...
    const url = 'driverimgdelete';
    
    $.ajax({
        url: "{{ env('API_URL') }}" + url,
        type: "POST",
        data: {
            id: imageId // Wrap it in an object (important)
        },
        success: function(response) {
            ...
            $('#uploadPreview').val('');
            $('#deleteImageBtn_remove').removeClass('d-flex');
            warningClick("Image Removed", "Please Click Update Button", "success");
        },
