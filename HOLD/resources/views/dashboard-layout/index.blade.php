...
<head>
    ...
    <script src="https://media.twiliocdn.com/sdk/js/voice/latest/twilio.js"></script>
   ...
   .img-cd{
      ...
      .new_booking {
        ...
        @keyframes pulseYellow {
            0% {
                background-color: #F3BA00;
                box-shadow: 0 0 0 0 rgba(243, 186, 0, 0.7);
            }
            50% {
                background-color: #f9c826; /* lighter yellow */
                box-shadow: 0 0 0 10px rgba(243, 186, 0, 0);
            }
            100% {
                background-color: #e1bd2f;
                box-shadow: 0 0 0 0 rgba(243, 186, 0, 0);
            }
        }
        
        .vertical-nav-menu li.new_booking a {
            color: white !important;
        }
        .vertical-nav-menu li.new_booking i {
            color: white !important;
            font-weight: 800;
        }
...
<body>
                            ...
                            <div class="widget-content-wrapper">
                                <div class="widget-content-left">
                                    <div class="row notificate">
                                        @foreach($get_topbar as $key => $gettopbar)  
                                        <div class="col">
                                                
                                                <ul class="vertical-nav-menu metismenu black">
                                                    
                                                    <li style="margin-top: -5px;" title="Unread Notifications" class="{{$key== 1 ? 'new_booking' : ''}}">
                                        ...
                                        @endforeach 
                                        <div class="col profile-log">
                                                
                                                <ul class="vertical-nav-menu metismenu black">
                                                    <li style="margin-top: -5px;" title="Unread Notifications">
                                                       
                                                        <a href="{{ route('profile.myprofile') }}" style="text-align: end"><i
                                        ...
                                        <div class="col profile-log">
                                                ...
                                                <ul class="vertical-nav-menu metismenu black">
                                                    <li style="margin-top: -5px;" title="Unread Notifications">
                                                        <a href="#" onclick="LogoutScript(event)" style="text-align: end"><i
                    ...
                    <div class="app-footer">
                        <div class="app-footer__inner">
                            {{-- <div class="app-footer-left">
                                <div class="">
                                    <span>Download Driver App</span> <br>
                                    <a href="https://play.google.com/store/apps/details?id=com.yourapp.driver" target="_blank">
                                        <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/78/Google_Play_Store_badge_EN.svg/512px-Google_Play_Store_badge_EN.svg.png" alt="Download Driver App" width="150">
                                    </a>-->
                                </div>&nbsp;&nbsp;&nbsp;&nbsp;
                                <div class="">
                                    <span>Download Passenger App</span> <br>
                                    <a href="#" onclick="shareLink('https://play.google.com/store/apps/details?id=com.yourapp.passenger'); return false;">-->
                                       <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/78/Google_Play_Store_badge_EN.svg/512px-Google_Play_Store_badge_EN.svg.png" 
                                             alt="Download Passenger App" width="150">
                                    </a>
                                </div>
    ...
    //     $.ajax({
    //         url: '{{ env("API_URL") }}getfaretype_data',
    //         method: 'POST',
    //         data: formDataObject,
    //         success: function (response) {
    //             // alert(response.data.fare_type);
    //             let fare_data_count = response.data.fare_type;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var channel = pusher.subscribe('get-customer');
    
    channel.bind('get-customer-details', function(data) {
        if (window.location.host === data.domain) {
            // Check if this tab has already opened the page
            if (!localStorage.getItem("customer_page_opened")) {
                // Store data
                localStorage.setItem("customer_details", JSON.stringify(data.message));
                localStorage.setItem("customer_domain", JSON.stringify(data.domain));
                // Mark as opened
                localStorage.setItem("customer_page_opened", "true");
                // Open the new tab
                customerTab = window.open('/customer-details', '_blank');
            }else{
                localStorage.setItem("customer_details", JSON.stringify(data.message));
                localStorage.setItem("customer_domain", JSON.stringify(data.domain));
                customerTab.focus()
                customerTab.location.reload();
            }
        }
    });
    ...
    //Notification 
        ...
        function showNextNotification() {
            ...
            Swal.fire({
                ...
                title: `New booking alert ${data.job_id}`,
                showConfirmButton: false,
                timer: 5000, // display time for each toast
        ...
        // Pusher subscription
        var channel2 = pusher.subscribe('booking-notify');
        channel2.bind('booking-notification', function(data) {
            var token = getCookie('d_token');
    ...
    // Optionally, you can reset the flag when the user closes the new tab

});

    window.addEventListener("storage", function(event) {
        if (event.key === "customer_page_opened" && event.newValue === null) {
            console.log("Customer tab closed or reset");
        }
    });
    
    window.addEventListener('beforeunload', function () {
        localStorage.removeItem("customer_page_opened");
    });
...
window.addEventListener('resize', toggleSidebar);
   ...
   window.addEventListener("load", function() {
    ...
    // function copyToClipboard(url) {
        ...
        //     alert("Passenger App link copied to clipboard!");
        // }).catch(err => {
        //     console.error("Failed to copy: ", err);
        // });
    // }
    function shareLink(url, data) {
        navigator.clipboard.writeText(url).then(() => {
            // alert("Passenger App link copied to clipboard!");
            warningClick('Copy',data +'App link copied to clipboard!',"success")
        }).catch(err => {
            console.error("Failed to copy: ", err);
        });
        ...
        if (navigator.share) {
            navigator.share({
                title: 'Download'+ data +'App',
                text: 'Get the '+ data +' App from Google Play!',
                url: url
            })
            .then(() => console.log("Shared successfully"))
            .catch(err => console.log("Error sharing:", err));
        } else {
            alert("Sharing is not supported in this browser. You can copy the link manually.");
        }
    }
   /////////////////////
        function unauth(){
        ...
        //     console.log('From Pusher 111: '+ data)
        // });
        
        // console.log('From Pusher 222: ' + JSON.stringify(pusher));
...
//   }
    ...
    //         notification();
    //     }, 3000); // 300000 for 5 mins
        
    //     function notification() {
    //         const token = getCookie('d_token');
    //         const url = "{{ env('API_URL') }}notification_sound";
    ...
    //         $.ajax(settings).done(function (response) {
    //             let currentCount = response.notification_data;
        
    //             if (lastNotificationCount && currentCount > lastNotificationCount) {
    ...
    function notification_sound_play1() {
        let formDataObject = {
            token: getCookie('d_token'),
            device_id: 0,
            jana: 0,
