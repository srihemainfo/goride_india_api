    ...
    <div class="card-body">
        <div class="row mb-4">
            <div class="col-sm-4">
                ...
                {{-- @if ($isEditable)
                    <input class="form-control" type="text" id="one_way_pick_up" name="one_way_pick_up"
                        value="{{ $isEditable && $booking_details->from ? $booking_details->from : '' }}">
                @else --}}
                    <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" tabindex="-1"
                        ...
                        <option value="{{ $isEditable && $booking_details->from ? $booking_details->from : '' }}">{{ $isEditable && $booking_details->from ? $booking_details->from : '' }}</option>
                    </select>
                {{-- @endif --}}
                <p class="text-danger invalid_one_way_pick_up"></p>
                <input type="hidden" name="from_place_id" id="from_place_id" value="{{ $isEditable && $booking_details->place_from ? $booking_details->place_from : '' }}">
            </div>

            <div class="col-sm-4">
                <label for="one_way_drop_off">Drop-off <span class="required">*</span></label>
                {{-- @if ($isEditable)
                    <input class="form-control" type="text" id="one_way_drop_off" name="one_way_drop_off"
                        value="{{ $isEditable && $booking_details->to ? $booking_details->to : '' }}">
                @else --}}
                    <select class="form-control select2 select2-hidden-accessible" style="width: 100%;" tabindex="-1"
                        aria-hidden="true" id="one_way_drop_off" name="one_way_drop_off" data-control="select2"
                        data-placeholder="Enter Airport, Seaport, Postcode" data-hide-search="true">
                        <option value="{{ $isEditable && $booking_details->to ? $booking_details->to : '' }}">{{ $isEditable && $booking_details->to ? $booking_details->to : '' }}</option>
                    </select>
                {{-- @endif --}}
                <p class="text-danger invalid_one_way_drop_off"></p>
                <input type="hidden" name="to_place_id" id="to_place_id" value="{{ $isEditable && $booking_details->place_to ? $booking_details->place_to : '' }}">
            </div>

            <div class="col-sm-4">
                <label for="one_way_ref_no">Reference No. </label>
                <input class="form-control" type="text" name="one_way_ref_no" id="one_way_ref_no"
                    placeholder="Outward ref no." value="{{ $isEditable && $booking_details->reference_no ? $booking_details->reference_no : '' }}">
            ...
            <div class="col-sm-12" id="pickup_point_container" style="display: {{ isset($booking_details->multi_pickup) && $booking_details->multi_pickup == "1" ? "block" : "none" }};">
                ...
                <div class="row mb-3" id="points_values">
                    @if (isset($pick_up_points ) && !empty($pick_up_points ))
                        @foreach ($pick_up_points as $pp)
                            <div class="col-sm-4 mb-2 d-flex justify-content-between location_row">
                                <input type="text" name="pickup_location[]" class="form-control from_location mr-1" value="{{ $pp->location_name }}">
        ...
        <div class="row mb-4">
            ...
            <div class="col-sm-2">
                ...
                <input class="form-control" type="time" id="one_way_pickup_time" name="one_way_pickup_time"
                    value="{{ $isEditable && $booking_details->pickup_time ? $booking_details->pickup_time : '' }}">
            ...
            <div class="col-sm-4">
                ...
                <input class="form-control" type="text" name="one_way_pickup_address" id="one_way_pickup_address"
                    ...
                    value="{{ $isEditable && $booking_details->pickup_address ? $booking_details->pickup_address : '' }}">
            ...
            <div class="col-sm-4">
                ...
                <input class="form-control" type="text" name="one_way_dropoff_address" id="one_way_dropoff_address"
                    placeholder="Full address with postcode"
                    value="{{ $isEditable && $booking_details->dest_address ? $booking_details->dest_address : '' }}">
        ...
        <div class="row mb-4 one_way_arrival_flight_ship_details">
            ...
            <div class="col-sm-3">
                ...
                @if ($isEditable && $booking_details->pickup_flight_num)
                    <input class="form-control" type="text" name="one_way_flight_number"
                        id="one_way_flight_number" placeholder="Flight Number"
                        value="{{ $booking_details->pickup_flight_num }}">
                @elseif($isEditable && $booking_details->pick_shipname)
                    <input class="form-control" type="text" name="one_way_flight_number"
                        id="one_way_flight_number" placeholder="Flight Number"
                        value="{{ $booking_details->pick_shipname }}">
                @else
                    <input class="form-control" type="text" name="one_way_flight_number"
        ...
        <div class="row mb-4 one_way_departure_flight_ship_details">
            <div class="col-sm-3">
                ...
                <input class="form-control" type="text" name="one_way_dest_ship_name"
                    ...
                    value="{{ $isEditable && $booking_details->dest_shipname ? $booking_details->dest_shipname : '' }}">
        ...
        <div class="row mb-4">
            ...
            <div class="col-sm-2">
                ...
                <select class="form-control" name="one_way_order_status" id="one_way_order_status">
                    @if($isEditable)
                        @if (strtolower($booking_details->order_status) === 'pending')
                            <option value="Pending">Pending</option>
                            <option value="Confirmed">Confirmed</option>
                            <option value="Canceled">Cancelled</option>
                        @elseif (strtolower($booking_details->order_status) === 'confirmed')
                            <option value="Pending">Pending</option>
                            <option value="Confirmed">Confirmed</option>
                            <option value="Canceled">Cancelled</option>
                        @elseif (strtolower($booking_details->order_status) === 'assigned')
                            <option value="Assigned">Assigned</option>
                            <option value="Dispatched">Dispatched</option>
                            <option value="Confirmed">Confirmed</option>
                            <option value="Canceled">Cancelled</option>
                        @elseif (strtolower($booking_details->order_status) === 'dispatched')
                            <option value="Dispatched">Dispatched</option>
                            <option value="Completed">Completed</option>
                            <option value="Canceled">Cancelled</option>
                        @elseif (strtolower($booking_details->order_status) === 'completed')
                            <option value="Completed">Completed</option>
                        @elseif (strtolower($booking_details->order_status) === 'settled')
                            <option value="settled">Settled</option>
                        @elseif (strtolower($booking_details->order_status) === 'canceled')
                            <option value="Pending">Pending</option>
                            <option value="Confirmed">Confirmed</option>
                            <option value="Canceled">Cancelled</option>
                        @endif
            ...
            <div class="col-sm-3">
                ...
                <input class="form-control text-success font-weight-bold" type="text" name="one_way_extra_cost"
                    ...
                    value="{{ $isEditable && $booking_details->car_park_amount ? $booking_details->car_park_amount : 0.0 }}">
            ...
            <div class="col-sm-2">
                ...
                <input class="form-control text-success font-weight-bold" type="text" name="one_way_distance"
                    ...
                    value="{{ $isEditable && $booking_details->distance ? $booking_details->distance : '' }}">
            ...
            <div class="col-sm-2">
                ...
                <input class="form-control text-success font-weight-bold" type="text" name="one_way_travel_time"
                    ...
                    value="{{ $isEditable && $booking_details->duration ? $booking_details->duration : '' }}">
                <p class="text-danger invalid_one_way_travel_time"></p>
            </div>

            @if($isEditable && in_array($booking_details->order_status, $driver_amount_field))
                <div class="col-sm-2">
                    <label for="one_way_driver_amount">Driver Amount</label>
                    <input class="form-control text-success font-weight-bold" type="text" name="one_way_driver_amount" id="one_way_driver_amount" value="{{ $isEditable && $booking_details->driver_amount ? $booking_details->driver_amount : '0.00' }}">
        ...
        <div class="row">
            <div class="col-sm-4">
                ...
                <textarea class="form-control" name="one_way_message" id="one_way_message" row="3">{{ $isEditable && $booking_details->message ? $booking_details->message : '' }}</textarea>
            </div>

            <div class="col-sm-4">
                <label for="one_way_remarks">Driver Remarks</label>
                <textarea class="form-control" name="one_way_remarks" id="one_way_remarks" row="3">{{ $isEditable && $booking_details->remarks ? $booking_details->remarks : '' }}</textarea>
            </div>

            <div class="col-sm-4">
                <label for="one_way_payment_message">Payment Message</label>
                <textarea class="form-control" name="one_way_payment_message" id="one_way_payment_message" row="3">{{ $isEditable && $booking_details->payment_message ? $booking_details->payment_message : '' }}</textarea>
            </div>
        </div>
    </div>
</div>
