<script>
    $(function() {
        ...
        $('#one_way_pick_up').change(function() {
            ...
            } else if ($('#one_way_pickup_address').val() === area) {
        ...
        $('#one_way_drop_off').change(function() {
            ...
            } else if ($('#one_way_dropoff_address').val() === area) {
        ...
        @if (!$isEditable)
            ...
            $('#return_pick_up').change(function() {
                ...
                } else if ($('#return_pickup_address').val() === area) {
            ...
            $('#return_drop_off').change(function() {
                ...
                } else if ($('#return_dropoff_address').val() === area) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                @if ($isEditable)
                    ...
                    for(let i = 0; i <= response[0].booster; i++){
                        child_dd += `<option value="${i}" ${i == {{ $booking_details->child_seat ? $booking_details->child_seat : 0  }} ? "selected" : ""}>${i}</option>`
                    }
                @else
    ...
    function CalculateAmount(journey_type) {
        let total_cost = parseFloat({{ $isEditable && $booking_details->total ? $booking_details->total : 0 }})
        ...
        if (journey_type === 'one_way') {
            $('#one_way_total_cost').val(Math.ceil(isEditable && one_way_actual_amount === 0 ? total_cost :
