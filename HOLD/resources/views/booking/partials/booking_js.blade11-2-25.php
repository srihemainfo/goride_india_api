<script>
    $(function() {
        ...
        $('#car_type, #one_way_pick_up, #one_way_drop_off').change(function() {
            ...
            if (
                from_area.includes('Airport') || 
                from_area.includes('terminal') || 
                to_area.includes('Airport') || 
                to_area.includes('terminal')
            ) {
            ...
            success: function(response) {
              ...
              //dd means dropdown
                ...
                for (let i = 1; i <= response['data'].passenger; i++) {
                    passenger_dd += `<option value="${i}" ${response['data'].passenger != null && response['data'].passenger == i ? 'selected' : ''}>${i}</option>`;
                }
                
                for (let i = 0; i <= response['data'].luggage; i++) {
                    luggage_dd += `<option value="${i}" ${response['data'].luggage != null && response['data'].luggage == i ? 'selected' : ''}>${i}</option>`;
                }
                
                for (let i = 0; i <= response['data'].hand_luggage; i++) {
                    hand_luggage_dd += `<option value="${i}" ${response['data'].hand_luggage != null && response['data'].hand_luggage == i ? 'selected' : ''}>${i}</option>`;
                }
                
                for (let i = 0; i <= response['data'].child; i++) {
                    child_dd += `<option value="${i}" ${response['data'].child != null && response['data'].child == i ? 'selected' : ''}>${i}</option>`;
                }
