...
$('.return_offershowhide').hide();
    ...
    $(function() {
        ...
        $('#car_type, #one_way_pick_up, #one_way_drop_off, #date, #date1, #one_way_pickup_time , #return_pickup_time, #tariff_return_pickup_date, #tariff_one_way_checkbox, #tariff_roundtrip_checkbox').change(function() {
            ...
            let tariff_return_date = $('#tariff_return_pickup_date').val();
            let tariff_type_hidden = $('#tariff_type_hidden_input').val();
            let tariff_type_val;
            ...
            if (car_type && from_area && to_area) {
                ...
                if(tariff_type_hidden == 'roundtrip'){
                    formDataObject['tariff_return_date'] = tariff_return_date;
                    tariff_type_val = 'roundtrip';
                }
                formDataObject['tariff_type'] = tariff_type_val;
                    ...
                    success: function(response) {
                        if (response.fare_type && response.fare_type.length > 0) {
                            ...
                            if (fareTypeValue == "2") {
                                ...
                                $('#km_base_show').addClass('d-none').hide();
                                $('#return_container').addClass('d-none').hide();
                                $('#tariff_base_show').removeClass('d-none').show();
                            } else {
                                $('#km_base_show').removeClass('d-none').show();
                                $('#tariff_base_show').addClass('d-none').hide();
    ...
    //Journey type selection for Tariff return container visibility
    $('[name="tariff_journey_type"]').click(function() {
        let journey_type = $('[name="tariff_journey_type"]:checked').val() == 'roundtrip';
        console.log('sdfghiolkjhg33',journey_type);
    
        if (journey_type) {
            $('#tariff_type_hidden_input').val('roundtrip');
            $('#tariff_return_date_show').removeClass('d-none').show();
        } else {
            $('#tariff_type_hidden_input').val('');
            $('#tariff_return_date_show').addClass('d-none').hide();
        }
    });
