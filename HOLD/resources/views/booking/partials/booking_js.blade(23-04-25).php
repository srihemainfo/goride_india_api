...
$('.return_offershowhide').hide();
            ...
            success: function(response) {
              ...
              //dd means dropdown
                ...
                for (let i = 1; i <= response['data'].passenger; i++) {
                    passenger_dd += `<option value="${i}" ${response['data'].passenger != null && response['data'].passenger == 0 ? 'selected' : ''}>${i}</option>`;
                }
                
                for (let i = 0; i <= response['data'].luggage; i++) {
                    luggage_dd += `<option value="${i}" ${response['data'].luggage != null && response['data'].luggage == 0 ? 'selected' : ''}>${i}</option>`;
                }
                
                for (let i = 0; i <= response['data'].hand_luggage; i++) {
                    hand_luggage_dd += `<option value="${i}" ${response['data'].hand_luggage != null && response['data'].hand_luggage == 0 ? 'selected' : ''}>${i}</option>`;
                }
                
                for (let i = 0; i <= response['data'].child; i++) {
                    child_dd += `<option value="${i}" ${response['data'].child != null && response['data'].child == 0 ? 'selected' : ''}>${i}</option>`;
                }
    ...
    function AssignValues(data, journey_type, currencyval) {
        ...
        let return_all_total = data.total_fare + data.r_time_price_only + data.r_day_price
