<script>
    $(function() {
        ...
        $('#book_search').on('click', function(){
            ...
            var lastSegment = segments.pop();
                 ...
                 formDataObject['order_status'] = lastSegment == 'All' ? '' : lastSegment;
            ...
            new DataTable('#book-table', {
             ...
             columns: [
                 ...
                 render: function(data,type,row){
                     return `<i class="fa-solid fa-building-columns" style="font-weight: 00;  margin-left: -125px; color: #114462;"></i> ${row.type}`;
                 }
                 },
                 { data: null,
                 className: 'dt-table-11',
                 render: function(data,type,row){
                     return `<i class="fa-solid fa-user" style="font-weight: 00;margin-left: -180px;color: #114462;"></i>${row.driver_name != null ? row.driver_name : '-'}`;
                 }
                 },
                 {
                     data: null,
    ...
    style="width: 100px !important; color: white; margin-right: 0; background-color: #82b600 !important;">Pending</button>
                         
                         <button class="form-control booking-status" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status" 
    style="   width:100px !important; background-color: ${row.order_status == 'Pending' ? '#fb6b05' : (row.order_status == 'Confirmed' ? '#07912b' : (row.order_status == 'Assigned' ? '#0e73c9' : (row.order_status == 'Dispatched' ? '#67138b' : (row.order_status == 'Completed' ? '#90ee90' : (row.order_status == 'Settled' ? '#897e22' : (row.order_status == 'Canceled' ? '#f33434' : 'inherit'))))))}; 
