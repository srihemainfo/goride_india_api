        ...
        $('#dash_book_search').on('click', function(){
            ...
            new DataTable('#dash-table', {
             ...
             columns: [
                 ...
                 render: function(data,type,row){
                     return `<i class="fa-solid fa-location-dot" style="font-weight: 00;margin: 0 9px 2px 4px;color: #114462;color: red;"></i>${row.from}`;
                 }
                 },
                 {  data: null,
                 ...
                 },
                 {
                     ...
                     render: function(data, type, row) {
                         ...
                         return `<div class="form-control booking-status" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status">
                       ${row.order_status == 'Pending' ? 'Pending' : ''}
                       ${row.order_status == 'Confirmed' ? 'Confirmed' : ''}
                       ${row.order_status == 'Assigned' ? 'Assigned' : ''}
                       ${row.order_status == 'Dispatched' ? 'Dispatched' : ''}
                       ${row.order_status == 'Completed' ? 'Completed' : ''}
                       ${row.order_status == 'Settled' ? 'Settled' : ''}
                       ${row.order_status == 'Canceled' ? 'Cancelled' : ''}
                    </div>`;
                     }
                 },
                 {
...
function showlist(status){
            ...
            new DataTable('#dash-table', {
             ...
             columns: [
                 ...
                 className: 'dt-table-3',
                 render: function(data,type,row){
                    return `<i class="fa-solid fa-calendar-days" style="font-weight: 600;margin: 0 9px 2px 4px;color: #114462;"></i>${row.pickup_date && row.pickup_time ? row.pickup_date + ' / ' + row.pickup_time : 'N/A'}`;
                }
                 },
                 { data: null,
                 className: 'dt-table-4',
                 render: function(data,type,row){
                    return `<i class="fa-solid fa-calendar-days" style="font-weight: 600;margin: 0 9px 2px 4px;color: #114462;"></i>${row.booking_date ? row.booking_date : 'Date not available'}`;
                }
                 },
                 { data: null,
                 className: 'dt-table-5',
                 render: function(data,type,row){
                    return `<i class="fa-solid fa-car" style="font-weight: 00;margin: 0 9px 2px 4px;color: #114462;"></i>${row.passengers ? row.passengers : '0'}`;
                }
                 },
                 { data: null,
                 className: 'dt-table-6',
                 ...
                 className: 'dt-table-9',
                 render: function(data,type,row){
                    return `<i class="fa-solid fa-user" style="font-weight: 00;margin: 0 9px 2px 4px;color: #114462;"></i>${row.driver_name != null ? row.driver_name : 'N/A'}`;
                 }
                 },
                 { data: 'type',
                 className: 'dt-table-10',
                 render: function(data,type,row){
                    return `<i class="fa-solid fa-file-invoice" style="font-weight: 00;margin: 0 9px 2px 4px;color: #114462;"></i>${row.payment_status != null ? row.payment_status : 'N/A'}`;
                }
                 },
                 { data: null,
                 className: 'dt-table-11',
                 render: function(data,type,row){
                    return `<i class="fa-solid fa-building-columns" style="font-weight: 00;margin: 0 9px 2px 4px;color: #114462;"></i>${row.type ? row.type : 'N/A'}`;
                 }
                 },
                 {
                     data: null,
        ...
        // return `<select class="form-control booking-status" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status" style="background-color: ${row.order_status == 'Pending' ? '#fb6b05' : (row.order_status == 'Confirmed' ? '#07912b' : (row.order_status == 'Assigned' ? '#0e73c9' : (row.order_status == 'Dispatched' ? '#67138b' : (row.order_status == 'Completed' ? '#90ee90' : (row.order_status == 'Settled' ? '#897e22' : (row.order_status == 'Canceled' ? '#f33434' : 'inherit'))))))}; color: ${row.order_status == 'Pending' || row.order_status == 'Confirmed' || row.order_status == 'Assigned' || row.order_status == 'Dispatched' || row.order_status == 'Completed' || row.order_status == 'Settled' || row.order_status == 'Canceled' ? 'white' : 'black'};">
        //     <option value="Pending" ${row.order_status == 'Pending' ? 'selected' : ''}>Pending</option>
        //     <option value="Confirmed" ${row.order_status == 'Confirmed' ? 'selected' : ''}>Confirmed</option>
        //     <option value="Assigned" ${row.order_status == 'Assigned' ? 'selected' : ''}>Assigned</option>
        //     <option value="Dispatched" ${row.order_status == 'Dispatched' ? 'selected' : ''}>Dispatched</option>
...
return `
     ...
     <button class="form-control booking-status me-5 bg-warning" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status" 
    style="width: 100px !important; color: white; margin-right: 0; background-color: #E0A008 !important;">Pending</button>

    <button class="form-control booking-status" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status" 
    style=" width:100px; background-color: ${row.order_status == 'Pending' ? '#fb6b05' : (row.order_status == 'Confirmed' ? '#07912b' : (row.order_status == 'Assigned' ? '#0e73c9' : (row.order_status == 'Dispatched' ? '#67138b' : (row.order_status == 'Completed' ? '#90ee90' : (row.order_status == 'Settled' ? '#897e22' : (row.order_status == 'Canceled' ? '#f33434' : 'inherit'))))))}; 
    color: ${row.order_status == 'Pending' || row.order_status == 'Confirmed' || row.order_status == 'Assigned' || row.order_status == 'Dispatched' || row.order_status == 'Completed' || row.order_status == 'Settled' || row.order_status == 'Canceled' ? 'white' : 'black'};">
        ...
        ${(row.order_status != 'Completed' && row.order_status != 'Settled') ? `
            <option value="Canceled" ${row.order_status == 'Canceled' ? 'selected' : ''}></option>
        ` : ''}
    </button>
     <button class="form-control booking-status ms-5" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status" 
    style="width: 100px !important; background-color: #F33434 !important; color: white; margin-right:0;">Canceled</button>
