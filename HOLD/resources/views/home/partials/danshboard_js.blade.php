<!--<audio id="notif-sound" src="{{ asset('assets/noti_sud.mp3') }}" preload="auto"></audio>-->
...
$(document).ready(function() {
    ...
    $.ajax(settings).done(function(response) {
        if (response && response.status === 200) {
            // alert("Currency: " + response.data.currency);
            var currencyString = response.data && response.data.currency ? response.data.currency : null;
            // var currencyString = response.data.currency;
            // // console.log("Jana Currency ",response.data);
            if (currencyString) {
...
// function redirectInvoice(id, job_no) {
//     let invoiceData = JSON.stringify({ id: id, job_no: job_no });
//     document.cookie = `invoice_booking=${invoiceData}; path=/`;
//     window.open("/invoice/create", "_blank");
// }
...
// sample data onclick="sendWhatsapp(1, '917305352593', 'Hii Jana Bro')"

// function passNumber(driverNumber, customerNumber) {
//     let driver_number = driverNumber;
//     let customer_number = customerNumber;
//     $('#send_driver_whatsapp_btn').attr('onclick', `sendWhatsapp('${driver_number}')`);
//     $('#send_customer_whatsapp_btn').attr('onclick', `sendWhatsapp('${customer_number}')`);
// }
...
function notification_sound_play() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (currentCount > lastNotifyCount1) {
...
// Run every 5 minutes (300000 ms), currently set to 10 seconds for testing
// let lastNotifyCount = 0; // Track previous count during this session
...
function showlist(status) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //         if (item.order_status === 'Pending') {
            //             document.getElementById('notif-sound').play();
            //             // warningClick('New Job Received', item.job_no, "success");
                        
            //             Swal.fire({
            //               toast: true,
            //               position: 'top-end', // top-right
            //               icon: 'success',
            //               title: `Job ${item.job_no} New Job Received`,
            //               showConfirmButton: false,
            //               timer: 7000,
            //               timerProgressBar: true,
            //               didOpen: (toast) => {
            ...
            // 🔔 Notify if job moved to "Completed" or "Moving" in last 5 mins
            // notifyList.forEach(item => {
            //     if (item.updated_at) {
            //         let updatedTime = new Date(item.updated_at);
            //         let diffInMin = (now - updatedTime) / 60000;
            ...
            //         if (diffInMin <= 5) {
            //             if (item.order_status === 'Completed') {
            //                 // warningClick('Job Completed', item.job_no, "success");
            ...
            //                 document.getElementById('notif-sound').play();
            //             } else if (item.order_status === 'Moving') {
            //                 // warningClick('Job Moving', item.job_no, "success");
...
function getColumns() {
    return [
                ...
                //  },
                 ...
                 render: function(data,type,row){
                    return `<div style="padding-bottom: 10px;"><i class="fa-solid fa-location-dot" title="To" style="font-weight: 00;margin: 0 9px 2px 4px;color: #114462;color: red;cursor: pointer;"></i>${row.to ? row.to : 'N/A'}`;
                }
                 },
                 {
                    data: null,
...

                 {
                     ...
                     render: function(data, type, row) {
                        ...
                        //     style="width: 108px !important; color: white; margin-right: 0; background-color: #0ecb02 !important;">Completed</button>`;
                        ...
                        //      <button class="form-control booking-status me-5 bg-warning" id="book_status${row.id}"  name="status" 
                        //     style="width: 108px !important; color: white; margin-right: 0; background-color: #0ecb02 !important;">Settled</button>`;
                            
                        //  } 
                        // if (orderStatus == 'Moving') {
                        //     return `
                    
                        //      <button class="form-control booking-status me-5 bg-primary" id="book_status${row.id}" onclick="changeBookStatus(${row.id}, this)" name="status" data-btn_name = "Completed" 
                        //     style="width: 108px !important; color: white; margin-right: 0; background-color: #02c0cb !important;">Moving</button>`;
                        ...
                        //      <button class="form-control booking-status me-5 bg-warning" id="book_status${row.id}" data-btn_name = "Canceled" name="status" 
                        //     style="width: 108px !important; color: white; margin-right: 0; background-color:  #F33434 !important;">Canceled</button>`;
                 ...
                 },
                 {
                     ...
                     render: function(data, type, row) {
                        ...
                        //  <li style="list-style-type: none;">   // drivermap
                        //     <a href="javascript:void(0)" onclick="drivermap(${row.id})" title="Driver Location" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-success">
                        //         <i class="fa fa-location-arrow"></i>
                        //     </a>
                        // </li> 

                        //  this is Download Button  <li  style="list-style-type: none;"><a href="/booking-status-pdf/${row.id}" target="_blank" title="Download PDF" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-dark downloadPDF"><i class="fa fa-download"></i></a></li>
                        // this is driver icon  ${row.order_status == 'Confirmed' ? `<li  style="list-style-type: none;"><a href="javascript:void(0)" title="Assign Driver" onclick="assigndriver(${row.id},'${row.job_no}','${row.total}')" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-success assignDriver"><i class="fa fa-user-plus"></i></a></li>` : ``}
                        // this is driver remove ${(row.order_status == 'Dispatched' || row.order_status == 'Assigned') ? `<li  style="list-style-type: none;"><a href="javascript:void(0)" onclick="removedriver(${row.id})" title="Remove Driver" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-danger removeDriver"><i class="fa fa-user-times"></i></a></li>
                         // <li  style="list-style-type: none;"><a href="javascript:void(0)" title="Send SMS"</a></li>` : ``}
                         
                        //  <li  style="list-style-type: none;"><button onclick="sendMail(${row.id}, 'invoice',  decodeURIComponent('${encodeURIComponent(JSON.stringify(row))}'))" target="_blank" title="Send Invoice" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-warning" style="padding: .25rem 0.6rem;"><i class="fa fa-file-text"></i></button></li>
                        //  ${(row.order_status == 'Dispatched' || row.order_status == 'Assigned') ? `<li  style="list-style-type: none;"><a href="javascript:void(0)" onclick="sendMail(${row.id}, 'others',  decodeURIComponent('${encodeURIComponent(JSON.stringify(row))}'))" title="Send Email" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-dark"><i class="fa fa-envelope"></i></a></li>` : ``}
                         return `<ul style="margin-right: -7px;margin-top: 90px;">
                         <li  style="list-style-type: none;"><a href="/booking/edit/${row.id}" title="Edit Job Details" class="mb-2 mr-2 btn-sm btn-transition btn btn-outline-primary editPrice"><i class="fa fa-edit"></i></a></li>
