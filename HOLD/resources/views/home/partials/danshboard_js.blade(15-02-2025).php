...
function showlist(status){
        ...
        // </select>`;
return `
    <select class="form-control booking-status" onchange="changeBookStatus(${row.id})" id="book_status${row.id}" name="status" 
    style="background-color: ${row.order_status == 'Pending' ? '#fb6b05' : (row.order_status == 'Confirmed' ? '#07912b' : (row.order_status == 'Assigned' ? '#0e73c9' : (row.order_status == 'Dispatched' ? '#67138b' : (row.order_status == 'Completed' ? '#90ee90' : (row.order_status == 'Settled' ? '#897e22' : (row.order_status == 'Canceled' ? '#f33434' : 'inherit'))))))}; 
