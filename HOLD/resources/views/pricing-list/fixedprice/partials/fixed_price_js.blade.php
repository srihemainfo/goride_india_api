...
function vehicletable() {
    ...
    $('#data-table').DataTable({
        ajax: {
            url: '{{env('API_URL')}}FixedPriceView',
            method: 'POST',
            dataSrc: function (json) {
                // Log the full response for debugging
                console.log('Server Response:', json);

                // Access the data using the 'data' key
                if (json && Array.isArray(json.data)) {
                    return json.data;
                } else {
                    console.error('Error: The `data` key is missing or is not an array.');
...
function EditVehicleValues(vehicleName) {
  ...
  $('#primaryBtn').click(function (e) {
    ...
    $.ajax({
        data: formdata,
        url: "{{env('API_URL')}}FixedPriceStore",
        type: "POST",
...
$(document).on('click', '.footable-edit', function() {
    ...
    $.ajax({
        url: '{{env('API_URL')}}FixedPriceEdit/edit/' + id, 
        method: 'POST',
        data: {
            id: id,  
            token: getCookie('d_token'),
        ...
        success: function(response) {
            if (response.status === 200) {
                ...
                $('#editpickup').val(data.pickup);
                $('#editdropoff').val(data.dropoff);
                $('#editcurrency').val(data.currency).trigger('change');
...
var vehicleId = $('#editVehicleId').val(); 
    ...
    var url = `{{env('API_URL')}}FixedPriceUpdate/update/` + vehicleId;
...
//         $.ajax({
//             url: '{{env('API_URL')}}FixedPriceDelete/delete/' + id,
