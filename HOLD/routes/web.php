...
$checkHost = DB::connection('mysql1')->table('gentral_setting')

    ->whereNotNull('cweburl')

    ->where('cweburl', $currentDomain)
    ...
    return response()
        ->json([
            'applinks' => [
                ...
                'details' => [
                    [
                        'appID' => 'Q8ANN98865.com.shi.myPassenger',
                        'paths' => ['/passenger/*']
                    ]
                ]
            ]
...
Route::get('testnewtab', function() {
    return response()
        ->json(['Name' => 'Mani Raj', 'Url' => 'https://ds406240668802.goride.run/dashboard'])
        ->header('Content-Type', 'application/json');
});
...
if (isset($checkHost) && $checkHost && in_array($checkHost->theme, ['ttwo', 'tthree', 'tone'])) {

    $currentTheme = $checkHost->theme ?? 'tone';

    Route::middleware(['seo'])->group(function () use ($checkHost, $currentTheme) {
        
        // dd($currentTheme);

        Route::get('/', function () use ($checkHost, $currentTheme) {

            return view($currentTheme . '.pages.dashboard')->with('theme', $currentTheme);

        })->name('home');
        // dd($currentTheme);
        Route::get('/bookingpreview/{bookingID}/{getText}', function ($bookingID, $getText) use ($checkHost, $currentTheme) {
            return view($currentTheme . '.pages.bookingpreview', [
                'bookingID' => $bookingID,
                'getText' => $getText
            ])->with('theme', $currentTheme);
        })->name('bookingpreview');


        // dd();
        Route::get('/{slug?}', function ($any = null) use ($checkHost, $currentTheme) {
            return view($currentTheme . '.pages.dashboard', ['slug' => $any])

                ->with('theme', $currentTheme);

        })->where('slug', '.*')->name('dynamic');
    ...
    $checkPartner = DB::connection('mysql1')->table('partnerlists')
        ...
        ->first();
    // dd($checkPartner);
    if (isset($checkPartner) && $checkPartner) {
        ...
        Route::middleware(['check.user'])->group(function () {
            ...
            // Route::get('/driver-documents/{id}/{name}', [DriverController::class, 'FileView'])->name('FileView');
            ...
            //Fare Setting Routes
            Route::get('/faresetting', [FaresettingController::class, 'faresetting']);
            ...
            Route::get('/temp-invoice', [InvoiceController::class, 'temp_invoiceIndex'])->name('temp-invoice');
            ...
            Route::get('mapzone', [MapzoneController::class, 'mapzone']);
            // Route::get('/admin-report', [ReportsController::class, 'ManageAdminReport'])->name('ManageAdminReport');
            ...
            Route::get('/faretype', [SettingsController::class, 'faretype']);
            
            
            //Tariff model
            Route::get('carrent-create', [SettingsController::class, 'tariff']);
            ...
            Route::get('/whatsapp-configuration', [SettingsController::class, 'whatsapp_configuration'])->name('whatsapp-configuration');
            
            Route::get('/call-configuration', [SettingsController::class, 'call_configuration'])->name('call-configuration');

            Route::get('/reviewdetails', [SettingsController::class, 'reviewdetails'])->name('reviewdetails');

            Route::get('/firstPaymentoption', [SettingsController::class, 'firstPaymentoption'])->name('firstPaymentoption');
            ...
            // Route::get('/create-carfares', function () {
            //     return redirect()->route('/carfares');
            // });
            ...
            //add new Job report
            Route::get('/job-report', [ReportsController::class, 'ManagejobReport'])->name('ManagejobReport');
            ...
            Route::get('/advance-payment', [SettlementController::class, 'advanceIndex'])->name('advanceIndex');
            ...
            Route::get('/email-builder', [PricingController::class, 'email_builder'])->name('email_builder');
