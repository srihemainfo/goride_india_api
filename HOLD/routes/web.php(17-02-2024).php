...
Route::middleware(['check.user'])->group(function () {
    ...
    return View::make('userprint', compact('sid'));
    ...
    Route::post('/check-special-day', [BookingController::class, 'CheckSpecialDay'])->name('CheckSpecialDay');
    ...
    Route::post('/email-settlements', [SettlementController::class, 'EmailSettlements'])->name('EmailSettlements');
...
Route::get('/query', function () {
    ...
    //         ->where('fare_details.status', '=', '1')
    //         ->whereNull('fare_details.deleted_at')
    //         ->select('fare_details.*', 'place.place')
    //         ->get();
    //     dd(DB::getQueryLog());

    //     $data = $data = DB::table('bookinfo')
    //     ->join('driver', 'driver.id', '=', 'bookinfo.driver_id')
    //     ->where('order_status', '=', 'Completed')
    //     ->select('bookinfo.*', 'driver.name')
    //     ->get();

    //     $quries = DB::getQueryLog();
    //     dd($quries);

    // $last_id = DB::table('invoiceno')
    //     ->orderby('id', 'desc')
    //     ->select('id')->limit(1)
    //     ->get();
    ...
    //     $inserts = [
    //         ['pound' => 1.25, 'euro' => 2.00, 'dollar' => 3.75],
    //         ['pound' => 1.25, 'euro' => 2.00, 'dollar' => 3.75],
    //         ['pound' => 1.25, 'euro' => 2.00, 'dollar' => 3.75],
    //         ['pound' => 1.25, 'euro' => 2.00, 'dollar' => 3.75],
    //         ['pound' => 1.25, 'euro' => 2.00, 'dollar' => 3.75],
    ...
    //         $ids[] = DB::table('currencies')->insertGetId($insert);
    //     }
    ...
    // dd(check_notification_exist('0493112412219203'));

    dd(Hash::make('test1234'));
    // event(new NotificationEvent('Booking ID: EC5678 is assigned for you. Accept or Decline a job asap.'));
});
