...
// $(document).ready(function() {
//     $('#accept_tc1').change(function() {
//         var pickupValue = $('#provider-remote1').val().trim();
//         var dropoffValue = $('#provider-remote2').val().trim();

//         // Check if both pickup and drop-off fields for main journey are not empty
//         if (pickupValue !== '' && dropoffValue !== '') {
//             if ($(this).is(':checked')) {
//                 $('#return-hidden').show();
//                 $('#provider-remote3').val(dropoffValue);
//                 $('#provider-remote4').val(pickupValue);
//             } else {
//                 $('#return-hidden').hide();
//             }
//         } else {
...
//             alert("Please fill in both the pickup and drop-off fields for the main journey.");
    ...
    $('#accept_tc1').change(function() {
        ...
        if ($(this).is(':checked')) {
            if (pickupValue !== '' && dropoffValue !== '') {
                $('#return-hidden').show();
                $('#provider-remote3').val(dropoffValue);
                if(dropoffValue=='Toronto Pearson International Airport canada'){
                    // alert('Pre-Arrange Pick-Up Location for both terminals, Door (A)');
            Swal.fire({
              title: 'Pre-Arrange Pick-Up Location for both terminals, Door (A)',
              icon: 'info',
              ...
              position: 'top-center'
                ...
                $('#provider-remote4').val(pickupValue);
                $('#provider-remote3').attr('required', 'required');
                $('#provider-remote4').attr('required', 'required');
            } else {
                alert("Please fill in both the pickup and drop-off fields for the main journey.");
                $('#accept_tc1').prop('checked', false); 
            }
            ...
            $('#return-hidden').hide();
            $('#provider-remote3').removeAttr('required');
            $('#provider-remote4').removeAttr('required');
        }
    });
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  timeInput.value = formatTime(currentTime.getHours(), currentTime.getMinutes());

  // Event listener to restrict input to valid time format
  timeInput.addEventListener("input", function () {
    ...
    if (value.length === 4) {
      ...
      if (minutes > 59) value = value.slice(0, 2) + "59";
    }

    this.value = formatTime(value.slice(0, 2), value.slice(2));
  });
});
