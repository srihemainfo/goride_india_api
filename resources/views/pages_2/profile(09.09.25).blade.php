                        ...
                        <div class="form-section">
                            ...
                            <div class="mb-1">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" id="f_name" placeholder="Enter full name" oninput="this.value = this.value.replace(/[^a-zA-Z]/g,'').slice(0,30)">
                                </div>
                                </div>
                                 <div class="col-6">
                                <div class="form-group">
                                    <label>Company Profile Name <span class="text-danger">*</span></label>
                                    <input type="text" name="comapny" id="f_comapny" placeholder="Enter Company name" oninput="this.value = this.value.replace(/[^a-zA-Z]/g,'').slice(0,30)">
                                </div>
                                </div>
                                </div>
                                <div class="form-group">
                                    <label>Email<span class="text-danger">*</span></label>
                                    <input type="email" name="email" id="f_email" placeholder="Enter email" oninput="this.value = this.value.replace(/[^0-9a-zA-Z]/g,'').slice(0,30)">
                                </div>
                                <div class="form-group">
                                    <label>Phone Number <span class="text-danger">*</span></label>
                                    <input type="tel" name="phone" id="f_phone" placeholder="Enter phone number" oninput="this.value = this.value.replace(/[^0-9]/g,'').slice(0,16)">
                                </div>
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" name="aadhar_no" id="aadhar_no"
                                        placeholder="e.g., Licence / Aadhaar / PAN Number" required oninput="this.value = this.value.replace(/[^0-9a-zA-Z]/g,'').slice(0,20)">
                                </div>
                                <div class="form-group mb-3">
                                            <label>Profile Image <span class="text-danger">*</span></label>
                                            <div class="profile-card  position-relative" onclick="document.getElementById('profileInput').click();">
                                                <img id="previewProfile" src="{{ asset('goride/img/proof_profile.png') }}" class="dummy-img ">
                                                <input type="file" id="profileInput" name="profile_image" accept="image/*" capture="user" class="d-none" onchange="previewImage(this,'previewProfile')">
                                                     <span class="plus-icon d-flex justify-content-center align-items-center ">+</span>
                                            </div>
                                        </div>
                                ...
                                <div class="row">
                                    <div class="col-md-12 col-12">
                                        <div class="form-group mb-3">
                                            ...
                                            <select class="form-select" name="id_proof_type" id="id_proof_type" required="">
                                    ...
                                    <div class="col-md-6 col-8">
                                        <div class="form-group mb-3">
                                            ...
                                            <div class="upload-card position-relative"
                                                ...
                                                <img id="previewFront"
                                                   src="{{ asset('goride/img/proof_front.png') }}"
                                                    class="dummy-img">
                                    ...
                                    <div class="col-md-6 col-8">
                                        <div class="form-group mb-3">
                                            ...
                                            <div class="upload-card position-relative"
                                                ...
                                                <img id="previewBack"
                                                          src="{{ asset('goride/img/proof_back.png') }}"
                                                    class="dummy-img">
                                    ...
                                    <small class="text-muted  d-block">
                                        <i class="fas fa-info-circle me-1"></i> File size must be between 1MB and 5MB. Allowed types: JPG, JPEG, PNG, WEBP.
                                    </small>
                                </div>
                            </div>
...
<script>
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status) {
                    ...
                    $('#id_proof_type').val(data.proof_type);
                    // $('#f_phone').val(data.mobile);
                    ...
                    if(data.aadhar_image_front != '' && data.aadhar_image_front){
                        

                        
                        $('#previewFront').attr('src', data.aadhar_image_front);
                        
                        $('#previewFront').nextAll('.plus-icon:first').css('display', 'none');
                    }else{
                        
                        $('#previewFront').nextAll('.plus-icon:first').css('display', 'block');
                        $('#previewFront').attr('src', "{{ asset('goride/img/proof_front.png') }}");
                    }
                    
                    if(data.aadhar_image_back != '' && data.aadhar_image_back){
                        
                        $('#previewBack').attr('src', data.aadhar_image_back);
                        
                        $('#previewBack').nextAll('.plus-icon:first').css('display', 'none');
                    }else{
                        
                        $('#previewBack').attr('src', "{{ asset('goride/img/proof_back.png') }}");
                        $('#previewBack').nextAll('.plus-icon:first').css('display', 'block');
                    }
                    
                    if(data.profile_img_url != '' && data.profile_img_url){
                        
                        $('#previewProfile').attr('src', data.profile_img_url);
                        $('#previewProfile').nextAll('.plus-icon:first').css('display', 'none');
                    }else{
                        
                        $('#previewProfile').attr('src', "{{ asset('goride/img/proof_profile.png') }}");
                        $('#previewProfile').nextAll('.plus-icon:first').css('display', 'block');
                    }
    ...
    function profile_update(event) {
        ...
        const requiredFields = [
            ...
            { name: "aadhar_no", label: "ID Proof Number ", type: "text" },
            { name: "id_proof_type", label: "ID Proof Type ", type: "text" },
            { name: "aadhar_image_front", label: "ID Proof Front", type: "file" },
            { name: "aadhar_image_back", label: "ID Proof Back", type: "file" },
        ...
        requiredFields.forEach(field => {
            ...
            let isEmpty = (field.type === "file") ? input.get(0).files.length === 0 : $.trim(input.val()) === "";

            if (field.name == 'aadhar_image_front') {
                var isPre_f = isEmpty ? $('#previewFront').attr('src'): input.val() ;
                // console.log(isPre_f)
                
            } else if (field.name == 'profile_image') {
                var isPre_f = isEmpty ? $('#previewProfile').attr('src') : input.val() ;
                
            } else if (field.name == 'aadhar_image_back') {
                var isPre_f = 'https://airportrides';
            ...
            if (isEmpty && !(isPre_f && isPre_f.includes('https://airportrides')) && field.name != 'aadhar_image_back') {
                
                errors.push(`${field.label} is required`);
                if (!firstInvalid) firstInvalid = input;
            }else{
                if (field.name == 'aadhar_image_front') {
                    isFront = input.get(0).files.length != 0 ? false : true;
                    
                } else if (field.name == 'profile_image') {
                    isPro = input.get(0).files.length != 0 ? false : true;
                    
                } else if (field.name == 'aadhar_image_back') {
                    isBack =  input.get(0).files.length != 0 ? false : true;
