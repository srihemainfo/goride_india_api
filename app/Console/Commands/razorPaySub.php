...
class razorPaySub extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $firstQuery = DB::table('subscriptions')
                ->whereNotNull('paymentStatus')
                // ->where('gateway', '!=', '')
                ->whereNotNull('subID')
                ->whereIn('gateway', ['razorpay', 'paypal'])
            ...
            $secondQuery = DB::table('subscriptions')
                ->whereNotNull('paymentStatus')
                // ->where('gateway', '!=', '')
                ->whereNotNull('subID')
                ->whereIn('gateway', ['razorpay', 'paypal'])
                // ->whereRaw('DATE(crontime) < CURDATE()')
                // ->whereRaw('createdon > NOW() - INTERVAL 1 DAY')
                
                      ->whereRaw('createdon > NOW() - INTERVAL 8 HOUR')
            ...
            $subscriptions = DB::table(DB::raw("({$firstQuery->toSql()}) as first"))
                ->mergeBindings($firstQuery)
                ->union(DB::table(DB::raw("({$secondQuery->toSql()}) as second")))
                ->mergeBindings($secondQuery)
                ->orderBy('crontime', 'asc')
                ->get()
                ->map(function ($item) {
                    ...
                    if (isset($subID) && $subID != null) {
                        ...
                        if ($value->gateway === 'razorpay') {
                            try {
                                ...
                                // Process subscription details here
                            } catch (\Razorpay\Api\Errors\BadRequestError $e) {
                                // Log the error instead of stopping the script
                                $this->line('Razorpay Subscription Fetch Error: ' . $e->getMessage());
                                // error_log("Razorpay Subscription Fetch Error: " . $e->getMessage());
                                
                                // Continue execution without stopping the cron job
                                $getSubscription = null; 
                            }
                                    ...
                                    if ($purchaseType === 'NEW' && isset($razStatus) && strtolower($razStatus) === 'active') {
                                        ...
                                        if ($oticket->count() < 1 && $invoiceRecheck->count() < 1) {
                                            ...
                                            $endDate = $checkout_response['expiryDate'];
                                                ...
                                                $findDays =   ($value->planType === 'YEARLY' ? $controller->getDaysInMonthOrYear('year', $checkout_response['productDetails']['validityMorY'])  :  $controller->getDaysInMonthOrYear('month', $checkout_response['productDetails']['validityMorY']));
                                                
                                            
                                                // ($value->planType === 'MONTHLY' ? Controller::getDaysInMonthOrYear('month', $checkout_response['noOfDays'])  : ($value->planType === 'YEARLY' ? Controller::getDaysInMonthOrYear('year', $checkout_response['noOfDays'])  : $plan->$validityKey ));
      
                                                
                                                
                                               $endDate =   Carbon::parse($checkout_response['startDate'])->addDays( $findDays )->format('Y-m-d');
                                                                                                                        ...
                                                                                                                        <td valign="top" align="center" style="padding:0;Margin:0;width:560px">
                                                                                                                            <table bgcolor="#ffa417" cellpadding="0" cellspacing="0" width="100%" style="mso-table-lspace:0pt;mso-table-rspace:0pt;border-collapse:collapse;border-spacing:0px;" role="presentation">
                                                                                                                                ...
                                                                                                                                <td align="center" bgcolor="#ffa417" style="padding:0;Margin:0;padding-top:20px;padding-bottom:10px">
                                                                                                                                    <p style="Margin:0;mso-line-height-rule:exactly;font-family:verdana, geneva, sans-serif;line-height:21px;letter-spacing:0;color:#ffffff;font-size:14px">Need Help? Visit us at <a href="' . (env('APP_URL') . '/') . '" style="color: white;">www.goride.net.in</a></p>
