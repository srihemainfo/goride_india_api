...
class CustomerAppController extends Controller
{
    ...
    private function sendAutoReply($payload)
    {
        try {
            ...
            $userMessage = strtolower(trim($messageData['text']['body'] ?? ''));
    
            // Default reply
            $reply = "Thanks for contacting GoRide 🚕";
    
            // 🎯 Keyword-based logic
            if (str_contains($userMessage, 'interested')) {
                $reply = "Great! 👍 Please reply *YES* to connect with our support team.";
            } 
            elseif ($userMessage === 'yes') {
                $reply = "Awesome! 🎉 Our support team will contact you shortly.\n\n📞 Call: 919094042940";
            } 
            elseif (str_contains($userMessage, 'price') || str_contains($userMessage, 'fare')) {
                $reply = "💰 Our fares are affordable and vary by distance.\n\nReply *BOOK* to get started.";
            } 
            elseif ($userMessage === 'book') {
                $reply = "🚕 Please share your pickup & drop location.";
            }
    ...
    private function applyFareLogic(
        ...
        $b_f = round($base + $commission);
        
        $from_range = 0;
        $to_range = 0;
        
        $tot_amt = round($b_f + $tax);
        
        $from_range_per = round($b_f * 0.1);
        $to_range_per = round($b_f * 0.1);
        
        $from_range = $b_f - $from_range_per;
        $to_range = $b_f + $to_range_per;
        ...
        try {
            ...
            if ($pickup->isToday() && $pickup->lessThan($now->copy()->addMinutes(90))) {
                return response()->json([
                    'status' => false,
                    'data' => [],
                    'message' => 'Pickup time must be at least 1.5 hours after the current time.'
                ]);
            }
...
//                     $previewFdUrl = env('PREVIEW_ENDPOINT') . 'fd/'. $shortCode;
...
// Check Available Drivers:- {$previewFdUrl}
...
//                     if ($u_details['mobile']) {
//                         $cleanPhoneCus = preg_replace('/[^0-9]/', '', $u_details['mobile']);
//                         if (strlen($cleanPhoneCus) === 10) {
//                             $cleanPhoneCus = '91' . $cleanPhoneCus;
//                         }

//                         $templateName = 'booking_from_website';
...
//                             'gateway' => 'fbWhatsapp',
//                             'subject' => 'Website Booking Confirmed',
//                             'details' => json_encode($params),
//                             'mobile' => $cleanPhoneCus,
//                             'ip' => $request->ip() ?? '',
//                             'datetime' => now(),
    ...
    try {
        ...
        if ($data['isDriver'] == 'no') {
            ...
            if ($check_data && !empty($data['pass_count'])) {
                ...
                if ($settings && $settings->default_discount > 0) {
                    $discountAmount = round(($base_fare * $settings->default_discount) / 100);
                    $base_fare -= $discountAmount;
                }
...
*GoRide System*";
                ...
                if ($u_details['mobile']) {
                    ...
                    $templateName = '_web_book_info_share_';
                    $template = DB::table('wamail_templates')->where('name', $templateName)->first();
                    $url = "https://graph.facebook.com/" . env('FB_WHATSAPP_VERSION', 'v24.0') . "/" . env('FB_WHATSAPP_PHONE_NUMBER_ID') . "/messages";
...
';
    ...
    public function apiBookingPreview(Request $request, $key)
    {
        $currentTime = now();
        ...
        $status = 'Pending';
        $now = now()->timezone('Asia/Kolkata');
        $pickupDateObj = Carbon::parse($job->pickup_date)->timezone('Asia/Kolkata');
    
        if ($job->job_status == 'accept' && $pickupDateObj > $now) {
            $status = 'Confirmed';
        } else if($pickupDateObj < $now && $job->job_status == 'accept'){
            $status = 'Completed';
        } else if($job->job_status == 'created' || $job->job_status == 'bidding'){
            $status = 'Pending';
        } else {
        ...
        if (!empty($job->pickup_date)) {
            try {
                $pickupDate = Carbon::parse($job->pickup_date)
                    ->format('d M Y, h:i A');
            } catch (\Exception $e) {
                $pickupDate = $job->pickup_date;
            }
        ...
        if (!empty($job->vehicle_image)) {
            ...
            if (is_array($vehicleDetails)) {
                foreach ($vehicleDetails as $k => $value) {
                    if (str_contains($k, '_image_url') && !empty($value)) {
                        $vehicleImages[] = $value;
                    }
                }
            }
        ...
        if(array_key_exists('total_fare', $fareBreakdown) && $fareBreakdown['total_fare'] == $job->pay_amt && $job->deductAmt == 0){
            ...
            $b_amt = $fareBreakdown['pay_to_driver'] ?? 0;
        } else {
            $b_amt = $baseFare + $tollFare;
        }
...
*GoRide System*";
    ...
    public function paymentBreakDown(Request $request)
    {
        try {
            ...
            // DB::rollBack();
            // \Log::error('Razorpay order creation failed: ' . json_encode($data));
        ...
        if($request->status && in_array($request->status, ['created', 'accept', 'cancelled'])){
            ...
            $book_history = DB::table('cus_job_temp as ct')
                ...
                ->select([
                    ...
                    DB::raw("
                        CASE
                            WHEN (ph.paymentStatus = 'success' AND ct.isDiscount = 'yes') OR (ph.gateway = 'cash')
                            THEN ct.credit
                            ELSE 0
                        END as credit
                    "),
    ...
    public function webPaymentBreakDown(Request $request)
    {
        try {
            ...
            \Log::info('Web Payment breakdown : ', [$e->getMessage()]);
    ...
    public function webCashOrder(Request $request)
   {
        ...
        try {
            ...
            if ($bidder_id) {
                ...
                $customerName = $u_d['name'] ?? 'Customer';
                $customerMobile = $u_d['mobile'] ?? '-';
    ...
    public function fetchBidding(Request $request, $key)
    {
        $currentTime = now();
        ...
        if ($job->user_id != 0) {
            return redirect('/booking-information/' . $key);
        }
        ...
        if (!in_array($job->job_status, ['created', 'bidding'])) {
            return redirect('/booking-information/'. $job->preview_hash);
        }
    
        return view('pages.fetchBidding', [
    ...
    public function apiFetchBidding(Request $request, $key)
    {
        $currentTime = now();
        ...
        // Instead of browser-redirecting inside an API, tell the app/client to redirect
        if ($job->user_id != 0) {
            return response()->json([
                'success' => true,
                'action' => 'redirect',
                'redirect_url' => '/booking-information/' . $key
            ], 200);
        }
        ...
        if (!empty($job->pickup_date)) {
            try {
                $pickupDate = Carbon::parse($job->pickup_date)->format('d M Y, h:i A');
            } catch (\Exception $e) {
                $pickupDate = $job->pickup_date;
            }
        }
        ...
        // Safety status checks wrapper logic converted for the API payload context
        if (!in_array($job->job_status, ['created', 'bidding'])) {
            return response()->json([
                'success' => true,
                'action' => 'redirect',
                'redirect_url' => '/booking-information/' . $job->preview_hash
            ], 200);
        }
