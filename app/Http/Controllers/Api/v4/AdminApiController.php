...
class AdminApiController extends Controller
{
    ...
    public function AdminbookJourney(Request $request)
    {
        try {
            $request->validate([
                ...
                'tax' => ['nullable', 'numeric'], // 🔥 FIXED: Validating 'tax', not 'tax_fare'
            ]);
            ...
            if (($data['isDriver'] ?? 'no') == 'no') {
                ...
                if ($check_data && !empty($data['pass_count'])) {
                    ...
                    // 🔥 FIX: Extract Tax exactly from the frontend payload's 'tax' key
    ...
    public function reject_bid_admin(Request $request)
    {
        try {
            ...
            // FIX: Use $get_job->user_id to find the job poster, NOT $request->uid
            ...
            // Safe fallback names just in case the user was deleted manually from DB
            $posterName = $user->name ?? 'Admin';
            $posterMobile = $user->mobile ?? 'Support';
            ...
            if ($get_job) {
                ...
                $amount = $job['bids_details'][$request->bidder_id]['amount'] ?? $get_job->fare ?? '0';
                ...
                $bidderName = $get_bidder->name ?? 'Driver';
...
Unfortunately, your bid was not selected by " . $posterName . " this time.  
...
Thanks for using " . env('APP_NAME') . ".  ";
                        ...
                        if ($accessToken) {
                            foreach ($fcmToken as $token) {
                                ...
                                $responses = $this->sendFCM(
                                    ...
                                    'Job ID ' . $get_job->job_no . ': Your bid has been rejected by ' . $posterName,
                                    [
                                        'caller' => $posterName,
    ...
    public function cancel_job(Request $request)
    {
        try {
            ...
            // Retrieve new cancellation parameters
            ...
            // =========================================================================
            // 🚀 FAST SYNC: Update MySQL DB IMMEDIATELY (Standard columns ONLY)
            // =========================================================================
            DB::table('cus_job_temp')
                ->where('user_id', $userId)
                ->where('job_no', $request->job_no)
            ...
            // =========================================================================
            // NEW LOGIC: ONLY Insert into job_cancellations table
            // =========================================================================
...
{
    \Log::info('--- CANCEL ASSIGNED JOB STARTED ---', $request->all());
    DB::beginTransaction();
    ...
    try {
        ...
        \Log::info("Processing cancellation for User ID: {$userId}, Job ID: {$request->job_id}, Job No: {$request->job_no}");

        if (!empty($request->user_id)) {
            if ($request->auth_key != 'ASDFGHJKLqwertyuiopMNBVCXZ!@#$%^&*()0987612345') {
                \Log::warning('Unauthorized attempt. Auth key mismatch.');
        ...
        if (!$get_job) {
            \Log::warning("Job not found in cus_job_temp for User: {$userId}");
            DB::rollBack();
            return response()->json(['status' => false, 'message' => 'Job Not Found'], 404);
        }

        \Log::info("Job found. Status: {$get_job->job_status}");
        ...
        try {
            ...
            if ($jobDoc) {
                ...
                $get_bidders_ids = array_keys($job['bids_details'] ?? []);
                $firebase->deleteJob($get_job->job_no);
                \Log::info("Firebase sync successful. Found " . count($get_bidders_ids) . " bidders.");
            } else {
                \Log::info("Job not found in Firebase.");
            }
        } catch (\Throwable $e) {
            \Log::error('Firebase Sync Failed during cancellation: ' . $e->getMessage());
        }
        ...
        if (empty($get_bidders_ids)) {
            ...
            \Log::info("Local fallback bidders count: " . count($get_bidders_ids));
        }
        ...
        \Log::info("Refund Check -> Now: {$now}, Pickup: {$pickupTime}, Difference: {$minutesDifference} mins.");

        if ($isEligibleForRefund) {
            \Log::info("Refund Check -> Eligible based on time (> 60 mins). Searching for successful payment...");
            ...
            if ($ch_payment) {
                \Log::info("Refund Check -> Payment found. ID: {$ch_payment->id}, GrandTotal: {$ch_payment->grandtotal}, WalletAmt: {$ch_payment->wallet_amt}. Checking for duplicates...");
                ...
                if (!$alreadyRefunded) {
                    \Log::info("Refund Check -> No duplicate found. Proceeding with wallet insert.");
                    ...
                    \Log::info("Refund Check -> Wallet successfully credited. Amount: {$expectedAmount}, New Balance: {$closingBalance}");
                } else {
                    \Log::warning("Refund Check -> SKIPPED. Wallet history already shows a refund for Payment ID: {$ch_payment->id}");
                }
            } else {
                \Log::warning("Refund Check -> SKIPPED. No successful payment found in payment_history for Job No: {$get_job->job_no} and User ID: {$userId}");
            }
        } else {
            \Log::warning("Refund Check -> SKIPPED. Time difference is less than 60 minutes ({$minutesDifference} mins).");
        }
        ...
        \Log::info("Job status updated to cancelled in all tables.");

        // WhatsApp Customer
        $request->job_type = $request->job_type ?? 'open';
        $get_u = DB::table($request->job_type == 'customer' ? 'customer_register' : 'user_register')
            ->where('id', $userId)
            ->where('deletes', '0')
            ->first();
        ...
        if ($get_u) {
            ...
            if ($existsWhatsApp) {
                // ... (Your WhatsApp logic remains the same)
                \Log::info("WhatsApp notification sent to Customer.");
            }
        }

        DB::commit();
        \Log::info('--- CANCEL ASSIGNED JOB COMPLETED SUCCESSFULLY ---');
        ...
        dispatch(function () use ($jobData, $biddersIds) {
            \Log::info("Dispatching notifications to bidders...");
            // ... (Your Dispatch logic remains the same)
        });
        ...
        DB::rollBack();
        \Log::error('Cancel Assigned Job CRITICAL Error: ' . $e->getMessage() . ' on line ' . $e->getLine());
        return response()->json([
            'status'  => false,
            'data'    => null,
    ...
    public function sendAdminPushNotification(Request $request)
    {
        ...
        app()->terminating(function () use ($controller, $title, $body, $hasUserIds, $userIds, $targetUser, $trackingId) {
            ...
            if (!empty($usersToNotify)) {
                ...
                if ($accessToken) {
                    ...
                    foreach ($usersToNotify as $user) {
                        ...
                        try {
                            $response = $controller->sendFCM($accessToken, $user['token'], $personalizedTitle, $personalizedBody, ['type' => 'admin_broadcast']);
...
";
    ...
    public function adminCancelScheduledJob(Request $request)
    {
        try {
            ...
            // 3. Fetch Customer Details (Scheduled jobs link to customer_register via user_id)
            ...
            // 4. Update the Job Status (Removed cancel_reason from here)
            \Illuminate\Support\Facades\DB::table('cus_job_temp')
                ->where('id', $jobId)
                ->update([
                    'job_status' => 'cancelled',
            ...
            // =========================================================================
            // 5. Insert into job_cancellations table
            // =========================================================================
            \Illuminate\Support\Facades\DB::table('job_cancellations')->insert([
                'job_id'       => $jobId,
                'customer_id'  => $job->user_id ?? 0,
            ...
            // 6. Construct & Send the WhatsApp Message
            try {
                if (!empty($customerPhone)) {
...
";
    ...
    public function adminWithdrawTransfer(Request $request)
    {
        ...
        // 3. Upload File to External API
        $fileNEW = '';
        if ($request->hasFile('filesent')) {
            $file = $request->file('filesent');

            // Validate extension & size locally first to save API calls
            ...
            try {
                ...
                if ($uploadResponse->successful() && isset($uploadData['status']) && $uploadData['status'] === true) {
                    $fileNEW = $uploadData['data']['url']; // Extract the URL from response
                } else {
                    return response()->json([
                        'type' => '0',
        ...
        // 4. Fetch Withdrawal Request
        $withdraw = DB::table('withdraw_request')
        ...
        // $withdraw_pk = $withdraw->id; // Not needed anymore since history logging was removed here
        ...
        try {
            ...
            $userQuery = DB::table($tableName)->where('id', $from_id)->where('deletes', '0');
            ...
            // $req_email = $userRow->email ?? ''; // Also no longer needed without the history insert

            // Update Request Status (Using the current balance since deduction already happened)
            ...
            // 6. Construct and Send WhatsApp Message
            if (strlen($req_mobile) === 10) {
                $req_mobile = '91' . $req_mobile;
            }

            $waMessage = "Hello {$req_fullname}, 👋\n\n";
            $waMessage .= "Your withdrawal request *{$request_id}* for ₹{$withdraw_amount} has been ✅ successfully transferred.\n\n";
            $waMessage .= "📌 Transaction Details:\n";
            $waMessage .= "🔹 Mode: {$mode}\n";
            $waMessage .= "🔹 Trans ID: {$transid}\n";
            $waMessage .= "It will reflect in your account within 24 hours.";
            ...
            // Send Notification (We ignore the true/false response now)
            Controller::sendNotification($whatsAppArr);

            // 7. Final check: Always Commit
            DB::commit();
    ...
    public function adminWithdrawReject(Request $request)
    {
        ...
        try {
            ...
            $user = DB::table($userTable)->where('id', $user_id)->where('deletes', '0')->first();
            ...
            // 5. Construct and Send WhatsApp Message
            if (strlen($req_mobile) === 10) {
                $req_mobile = '91' . $req_mobile;
            }

            $waMessage = "Hello {$req_fullname}, 👋\n\n";
            $waMessage .= "Your withdrawal request *{$request_id}* for ₹{$amt} has been ❌ rejected.\n\n";
            $waMessage .= "📌 Reason:\n{$reason}\n\n";
   ...
   public function resendPushNotification(Request $request)
    {
        ...
        app()->terminating(function () use ($controller, $title, $body, $userIds, $originalId, $originalResJsonStr, $target) {
            ...
            // Select table based on Target
            if ($target === 'Drivers' || $target === 'RM Users') {
                ...
                $users = DB::table('customer_register')
                    ->whereIn('id', $userIds)
                    ->where('deletes', '0')
                    ->select('id', 'name', 'fcm_token')
                    ->get();
    ...
    public function requestTemplateApproval(Request $request)
    {
        ...
        // Decode HTML entities (important for emojis & special chars)
        $cleanBody = html_entity_decode($cleanBody, ENT_QUOTES | ENT_HTML5, 'UTF-8');

        // Clean extra spaces but keep line breaks
        ...
        // Optional: decode HTML entities like &nbsp; if they appear as weird text in WhatsApp
        ...
        // Initialize components array with the BODY
        ...
        if (!empty($template->variables_json)) {
            ...
            // Check if there are buttons configured and the type is not 'none'
            if (isset($buttonsConfig['type']) && $buttonsConfig['type'] !== 'none' && !empty($buttonsConfig['buttons'])) {
                ...
                foreach ($buttonsConfig['buttons'] as $btn) {
                    if ($btn['type'] === 'QUICK_REPLY') {
                        $fbButtonsArray[] = [
                            "type" => "QUICK_REPLY",
                            "text" => mb_substr($btn['text'], 0, 25) // Meta strict limit: 25 chars
                        ];
                    } elseif ($btn['type'] === 'URL') {
                        $fbButtonsArray[] = [
                            "type" => "URL",
                        ...
                        $fbButtonsArray[] = [
                            "type" => "PHONE_NUMBER",
                            "text" => mb_substr($btn['text'], 0, 25),
                            "phone_number" => $btn['phone_number'] // E.g., +919876543210
                        ];
                    }
                }

                // If we successfully built buttons, push them to the components array
        ...
        // 4. Final Payload mapped for Meta
        $payload = [
            "name" => $fbTemplateName,
        ...
        try {
            ...
            if ($response->successful() && isset($body['id'])) {
                ...
                // Include Meta's detailed error message for easier debugging
    ...
    public function sampleMess(Request $request)
    {
        $request->validate([
            'mobile' => 'required',
            'template_name' => 'required', // Name comes dynamically from dropdown
            'message_body' => 'required'   // Used strictly for your DB insert
        ]);

        // WhatsApp requires country code without + or spaces
        $mobile = preg_replace('/[^0-9]/', '', $request->mobile);

        $url = "https://graph.facebook.com/" .
            env('FB_WHATSAPP_VERSION') . "/" .
            env('FB_WHATSAPP_PHONE_NUMBER_ID') . "/messages";

        // 1. Build the dynamic parameters array for Facebook
        $parameters = [];
        ...
        // 2. Build the Payload exactly in the format you requested
        $payload = [
            "messaging_product" => "whatsapp",
            "to" => $mobile,
            "type" => "template",
            "template" => [
                "name" => $request->template_name, // Dynamic template name
                "language" => [
                    "code" => $request->template_language ?? "en_US"
                ],
                "components" => [
                    [
                        "type" => "body",
                        "parameters" => $parameters // Dynamic parameters mapped here
                    ]
                ]
            ]
        ];
        ...
        // LOG INTO YOUR TABLE (UNTOUCHED LOGIC)
        DB::table('smslog')->insert([
            ...
            'smsstatus' => $response->successful() ? 'Sent' : 'Failed',
            'smssendstatus' => $response->successful() ? '1' : '0',
        ...
        return response()->json([
            ...
            'message' => $response->successful() ? 'Message Sent Successfully' : ('Failed: ' . ($body['error']['message'] ?? 'Unknown Error'))
            // 'full_response' => $body
        ]);
    }
    ...
    public function sendFCM($accessToken, $fcmToken, $title, $body, $data = [])
    {
        ...
        $payload = [
            ...
            'message' => [
                'token' => $fcmToken,
                'notification' => [ // ðŸ‘ˆ required for mobile push
                    'title' => $title,
                    'body' => $body,
                ],
                'android' => [
    ...
    public function adminUpdateJob(Request $request)
{
    try {
        ...
        // Fetch Old Job Early to sync fare JSON
        $oldJob = \Illuminate\Support\Facades\DB::table('cus_job_temp')->where('id', $jobId)->first();
        if (!$oldJob) {
            return response()->json(['status' => false, 'message' => 'Job not found.'], 404);
        }

        // --- START: FARE BREAKDOWN SYNC LOGIC ---
        $fareBreakdown = [];
        if (!empty($oldJob->fare_breakdown) && is_string($oldJob->fare_breakdown)) {
            $fareData = json_decode($oldJob->fare_breakdown, true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($fareData)) {
                $fareBreakdown = $fareData;
            }
        }

        // Safe Variable Extraction using Laravel's input() to ensure we catch the new form values
        $taxAmount = $request->input('tax') !== null && $request->input('tax') !== '' ? (float)$request->input('tax') : (float)($fareBreakdown['tax'] ?? $oldJob->tax ?? 0);
        $baseFare  = $request->input('base_fare') !== null && $request->input('base_fare') !== '' ? (float)$request->input('base_fare') : (float)($fareBreakdown['base_fare'] ?? $oldJob->base_fare ?? 0);
        $tollFare  = $request->input('toll_fare') !== null && $request->input('toll_fare') !== '' ? (float)$request->input('toll_fare') : (float)($fareBreakdown['toll_fare'] ?? $oldJob->toll_fare ?? 0);
        $totalFare = $request->input('fare') !== null && $request->input('fare') !== '' ? (float)$request->input('fare') : (float)($fareBreakdown['total_fare'] ?? $oldJob->fare ?? 0);

        // Overwrite the JSON breakdown with the Admin's new inputs
        ...
        // Preserve commission and discount stats if they exist so preview logic doesn't break
        if (!isset($fareBreakdown['com'])) $fareBreakdown['com'] = (int)($oldJob->com ?? 0);
        if (!isset($fareBreakdown['discount'])) $fareBreakdown['discount'] = (int)($oldJob->discount ?? 0);
        if (!isset($fareBreakdown['isDiscount'])) $fareBreakdown['isDiscount'] = $oldJob->isDiscount ?? 'no';
        // --- END: FARE BREAKDOWN SYNC LOGIC ---

        // Format Job Type for DB consistency
        $jobType = $request->job_type === 'round_trip' ? 'roundtrip' : 'oneway';

        // Map cab_type to human-readable format
        $cabNameMap = [
            'mini'         => 'Go Mini',
        ...
        // 2. Prepare user_details JSON array
        $userDetails = [
            'name'       => $request->c_name,
            'email'      => $request->c_email ?? '',
            'mobile'     => $request->c_mobile,
            'pass_count' => (int)($request->pass_count ?? 1),
        ...
        foreach ($fieldsToCheck as $field => $newValue) {
            ...
            if (empty($oldStr) && empty($newStr)) {
                continue;
            }

            if ($field === 'pickup_date') {
                if (date('Y-m-d H:i', strtotime($oldStr)) === date('Y-m-d H:i', strtotime($newStr))) {
                    continue;
                }
            }

            if (in_array($field, ['base_fare', 'toll_fare', 'fare', 'distance', 'tax', 'without_tax'])) {
                if (floatval($oldStr) === floatval($newStr)) {
                    continue;
                }
            }
        ...
        // Check JSON User Details Changes
        ...
        // Save Log to DB if there are changes
        if (!empty($changes)) {
        ...
        // 3. Base Update Array for cus_job_temp
        $updateData = [
            'job_type'       => $jobType,
            ...
            'without_tax'    => $baseFare,
            'fare'           => $totalFare,
            'fare_breakdown' => json_encode($fareBreakdown), // <--- SAVING UPDATED JSON HERE
            'updated_at'     => now(),
        ];

        /* =========================
           STRICT USER ROUTING LOGIC
        ========================== */
        ...
        $firebaseUpdateData = [
            ...
            'fare_breakdown' => ['stringValue'  => json_encode($fareBreakdown)], // Syncing JSON to Firebase
            ...
            'pass_count'     => ['integerValue' => (string)(int)($request->pass_count ?? 1)],
        ...
        // Build updateMask for Firestore PATCH request
        $updateMask = [];
        ...
        $payload = ['fields' => (object)$firebaseUpdateData];
    ...
    public function getExpiredJobs(Request $request)
    {
        try {
            ...
            $query = \Illuminate\Support\Facades\DB::table('cus_job_temp')
                ->where('deletes', '0')
                ->whereNotIn('job_status', ['accept', 'accepted', 'completed', 'cancelled'])
            ...
            // 2. Collect IDs to prevent N+1 queries
            $driverIds = [];
            $customerOwnerIds = [];
            $userOwnerIds = [];
            ...
            foreach ($rawJobs as $item) {
                ...
                if ($userId > 0) {
                    ...
                    // Both 'customer' and 'schedule' jobs belong to customer_register
            ...
            $jobs = $rawJobs->map(function ($item) use ($driverNames, $driverMobiles, $customerDetails) {
                $job = (array) $item;
                $gType = strtolower($job['global_type'] ?? '');
                ...
                // Priority: DB Profile -> JSON Details -> Root Row
                ...
                // Set standardized root keys for Frontend mapping
                ...
                // 🔥 CRITICAL FIX: Leave user_details exactly as-is so Frontend JS properly classifies "Customer App"
                if (empty($details)) {
                    $job['user_details'] = '';
                } else {
                ...
                $baseVal = !empty($job['base_fare']) ? $job['base_fare'] : 0;
                $tollVal = !empty($job['toll_fare']) ? $job['toll_fare'] : 0;
                ...
                // --- DECODE SCHEDULE STATUS (Like your reference method) ---
                $job['sch_amount'] = 0;
                $job['sch_status_data'] = [];
                ...
                // Set final fare for schedule jobs and hide breakdown fields entirely
                // Set final fare for schedule jobs and hide breakdown fields entirely
                if ($gType === 'schedule') {
                    if ($job['sch_amount'] > 0) {
                ...
                // Fetch push notification success count directly using cus_job_temp ID
                $job['count'] = 0;
                ...
                if ($searchJobId > 0) {
                    ...
                    if ($pushRecord && !empty($pushRecord->res_json)) {
                        ...
                        if (isset($resData['success_count'])) {
                            $job['count'] = (int)$resData['success_count'];
                        }
                    }
                }
   ...
   public function admin_accept_job(Request $request)
{
    ...
    try {
        ...
        $user_d   = (int) $request->u_id;    // The Actual Customer/User ID
        $jobNo    = $request->job_no;

        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 2 - Validation passed. Fetching job details from DB.");

        /* ================= FETCH CUS_JOB_TEMP EARLY ================= */
        $get_log = \Illuminate\Support\Facades\DB::table('cus_job_temp')->where('job_no', $jobNo)->first();
        
        if (!$get_log) {
            \Log::warning("AdminAcceptJob [JobNo: {$jobNo}]: Failed - Job not found in database.");
            return response()->json(['status' => false, 'message' => 'Job not found in database.'], 200);
        }

        /* 🔒 INTEGRITY CHECKS */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 3 - Running integrity checks.");
        
        if ($get_log->user_id != $user_d) {
            \Log::warning("AdminAcceptJob [JobNo: {$jobNo}]: Failed - Unauthorized action. Request u_id ({$user_d}) does not match job user_id ({$get_log->user_id}).");
        ...
        if (in_array($get_log->job_status, ['accept', 'completed', 'cancelled'])) {
            \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Stopped - Job is already in status: {$get_log->job_status}.");
        ...
        if ($alreadyAssigned) {
            \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Stopped - Job already exists in open_jobs table.");
            return response()->json(['status' => false, 'message' => 'Job has already been assigned to a driver.'], 200);
        }

        /* ================= FETCH USER ================= */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 4 - Fetching Customer Data.");
        ...
        if (!$user && $user_d != 0) {
            \Log::warning("AdminAcceptJob [JobNo: {$jobNo}]: Failed - User ID {$user_d} not found in {$tableName}.");
            return response()->json(['status' => false, 'message' => 'User not found in ' . $tableName], 200);
        }

        /* ================= FIREBASE ================= */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 5 - Fetching Job from Firebase.");
        $firebase = new \App\Services\FirebaseJobService(
            $this->serviceAccount['project_id'],
            $this->getAccessToken()
        );
        ...
        if (!$firebaseDoc) {
            \Log::warning("AdminAcceptJob [JobNo: {$jobNo}]: Failed - Job not found in Firebase.");
        ...
        if (empty($job['bids_details']) || !isset($job['bids_details'][$bidderId])) {
            \Log::warning("AdminAcceptJob [JobNo: {$jobNo}]: Failed - Bidder ID {$bidderId} not found in Firebase bids_details.");
        ...
        /* ================= FLAWLESS FARE MATHEMATICS ================= */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 6 - Calculating UI-Aligned Fare Mathematics.");
        ...
        // Calculate exact values matching the paymentBreakDown logic
        ...
        if ($base_fare < 0) $base_fare = 0; // Failsafe

        // 1. Commission is 5% of (Base + Toll)
        $com = (int) round(($base_fare + $toll_fare) * 0.05);

        // 2. Tax is 5% of (Base + Com)
        $tax = (int) round(($base_fare + $com) * 0.05);

        // 3. Total Fare accounts for everything so UI math perfectly sums up
        $total_fare = $base_fare + $tax + $com + $toll_fare;

        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Fare Math Verified -> Total Fare: {$total_fare} == Base({$base_fare}) + Tax({$tax}) + Com({$com}) + Toll({$toll_fare}). Driver gets FULL amount.");

        /* ================= TARIFF & CAB SEATING ================= */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 7 - Determining Tariff and Vehicle Settings.");
        ...
        /* ================= JSON DATA BREAKDOWN ================= */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 8 - Structuring Breakdown Data.");
        $data = [
            'id'              => $job['id'] ?? null,
            'bidder_id'       => $bidderId,
            'job_no'          => $jobNo,
            ...
            // STRICTLY COMPUTED FARES
            'base_fare'       => $base_fare,
            'toll_fare'       => $toll_fare,
            'tax'             => $tax,
            'com'             => $com,
            'pay_to_driver'   => $total_fare, // 🔥 Driver gets full amount
            'total_fare'      => $total_fare,
            'part_pay_fare'   => $com,
            'credit_pay_fare' => 0,           // 🔥 No Discounts
            'isDiscount'      => 'no',        // 🔥 No Discounts
            'inclusion'       => $inclu,
            'exclusion'       => $exclu
        ];

        $depositAmt = $total_fare;
        $tran_id = 'GRB' . time() . rand(1000, 9999);

        /* ================= DATABASE TRANSACTION ================= */
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 9 - Beginning DB Transaction.");
        ...
        try {
            ...
            \Illuminate\Support\Facades\DB::table('cus_job_temp')
                ...
                ->update([
                    ...
                    'isDiscount'     => 'no',    // 🔥 Forced to 'no'
                    'deductAmt'      => $com,
                    'credit'         => 0,       // 🔥 Forced to 0
                    'fare_breakdown' => json_encode($data),
                    "bids_details->{$bidderId}->status" => 'accept',
                    'updated_at'     => now()
                ]);
            ...
            // 4. Migrate to Open Jobs
            $openJobId = \Illuminate\Support\Facades\DB::table('open_jobs')->insertGetId([
                'job_no'         => $jobNo,
                'user_id'        => $user_d,
                'assigned_to'    => $bidderId,
                'fare'           => $total_fare,
                'distance'       => $job['distance'] ?? 0,
                'pickup_date'    => $get_log->pickup_date,
                'dropoff_date'   => $get_log->dropoff_date ?? null,
                'day'            => $get_log->day ?? '',
                'from_place'     => $job['from_place'] ?? '',
                'to_place'       => $job['to_place'] ?? '',
                'job_type'       => $job['job_type'] ?? '',
                ...
                'discount'       => 0,      // 🔥 Forced to 0
                'isDiscount'     => 'no',   // 🔥 Forced to 'no'
                'wallet_amt'     => $get_log->wallet_amt ?? 0,
                'credit'         => 0,      // 🔥 Forced to 0
            ...
            \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 10 - DB Transaction Committed Successfully.");

        } catch (\Exception $e) {
            \Illuminate\Support\Facades\DB::rollBack();
            \Log::error("AdminAcceptJob [JobNo: {$jobNo}]: Transaction Failed! Rolled back. Error: " . $e->getMessage());
            return response()->json([
                'status'  => false,
        ...
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Step 11 - Executing Post-Transaction Sync (Firebase & Push Notifications).");
        
        try {
            $firebase->updateBidStatus($jobNo, $bidderId, 'accept');
            $firebase->updateJobStatus($jobNo, 'accept');
            \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Firebase doc updated.");
            ...
            if ($user) {
                ...
                \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Invoice Job Dispatched.");
            }
                ...
                if ($accessToken) {
                    ...
                    foreach ($fcmToken as $token) {
                        try {
                            ...
                            \Log::error("AdminAcceptJob [JobNo: {$jobNo}]: FCM send error for token {$token}: " . $e->getMessage());
                        }
                    }
                    \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Push notifications processed.");
                }
            }
        } catch (\Throwable $e) {
            \Log::error("AdminAcceptJob [JobNo: {$jobNo}]: Post-DB API Sync error: " . $e->getMessage());
        }

        // Final cleanup
        $firebase->deleteJob($jobNo);
        \Log::info("AdminAcceptJob [JobNo: {$jobNo}]: Process Complete. Job assigned successfully.");
        ...
        \Log::warning("AdminAcceptJob: Validation Exception", $e->errors());
        return response()->json([
            'status'  => false,
            'message' => 'Validation failed.',
            'errors'  => $e->errors()
        ...
        \Log::critical("AdminAcceptJob [JobNo: {$jobNo}]: FATAL EXCEPTION: " . $e->getMessage(), ['trace' => $e->getTraceAsString()]);
        return response()->json([
            'status'  => false,
    ...
    public function adminJobList(Request $request)
    {
        try {
            ...
            // Remove status filter early so base filter logic doesn't hide everything
            ...
            $jobs = array_values(array_filter($jobs, function ($job) use ($filters, $baseFilters, $currentTime, $reqStatus) {
                if (!$this->applyFilters($job, $baseFilters)) {
                    return false;
                }
            ...
            // Bulk Fetch Drivers & Agents (from user_register)
            $allUserRegisterIds = array_unique(array_merge($bidDriverIds, $userOwnerIds));
            $userRegisterDetails = [];
            if (!empty($allUserRegisterIds)) {
                $userRegisterDetails = \Illuminate\Support\Facades\DB::table('user_register')
                    ->whereIn('id', $allUserRegisterIds)
                    ->select('id', 'name', 'mobile', 'roll_id')
                    ->get()
                    ->keyBy('id')
                    ->toArray();
            }

            // Bulk Fetch Customers (from customer_register)
            ...
            foreach ($jobs as &$job) {
                ...
                // Safely parse user_details if it comes as a JSON string
                $details = [];
                ...
                // Initialize Root Keys for frontend formatting
                $job['name'] = "";
                $job['mobile'] = "";
                $job['type'] = "";
                $job['kd_id'] = null;
                ...
                // Populate Root Keys strictly from Database Lookup
                if ($userId > 0) {
                    if ($gType === 'customer') {
                    ...
                    // Sync u_name and u_mobile with DB values
                    $job['u_name']   = $job['name'];
                    $job['u_mobile'] = $job['mobile'];
                }

                // Fill luggage and cab details
                $job['lugg_count'] = $job['lugg_count'] ?? $job['luggage'] ?? ($details['lugg_count'] ?? 0);
                $job['pass_count'] = $job['pass_count'] ?? ($details['pass_count'] ?? 1);
                $job['cab_type']   = $job['cab_type'] ?? $job['car_type'] ?? ($details['cab_type'] ?? '');

                // Fetch push notification success count from push_notifications table
                $job['count'] = 0;
                $currentStatus = strtolower($job['job_status'] ?? '');
                $searchJobId = (int)($job['id'] ?? 0);
                
                if ($currentStatus === 'accept' || $currentStatus === 'accepted') {
                    // 1. Take job_no from open_jobs
                    $openJob = \Illuminate\Support\Facades\DB::table('open_jobs')
                        ->where('id', $searchJobId)
                        ->first(['job_no']);
                        
                    if ($openJob && !empty($openJob->job_no)) {
                        // 2. Find id in cus_job_temp using job_no
                        $cusJobTemp = \Illuminate\Support\Facades\DB::table('cus_job_temp')
                            ->where('job_no', $openJob->job_no)
                            ->first(['id']);
                            
                        if ($cusJobTemp) {
                            $searchJobId = $cusJobTemp->id;
                        }
                    }
                }

                // 3. Search push_notifications req_json for the job_id and extract success_count from res_json
                if ($searchJobId > 0) {
                    $pushRecord = \Illuminate\Support\Facades\DB::table('push_notifications')
                        ->where('req_json', 'LIKE', '%"job_id": ' . $searchJobId . '%')
                        ->orWhere('req_json', 'LIKE', '%"job_id":' . $searchJobId . '%')
