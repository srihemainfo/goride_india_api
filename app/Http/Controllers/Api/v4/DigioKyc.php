...
class DigioKyc extends Controller
{
    ...
    function generateUniqueId($column, $type = 'alphanumeric', $length = 12, $txt = null)
    {
        if($txt == 'digio'){
            ...
            } while (DB::connection('mysql_log')->table('ocr_request')->where($column, $random)->exists());
        }
    ...
    public function boot_status(Request $request){
        ...
        try {
            ...
            $getlastJob = DB::table('cus_job_temp')
                ...
                ->whereIn('global_type', ['dr_carpool', 'open', 'schedule'])
                ->latest('created_at')
                ->first();
            
            $get_kyc->per_km = auth()->user()->per_km;
            $get_kyc->sch_from_place = null;
            $get_kyc->sch_from_place_id = null;
            
            $get_kyc->sch_to_place   = null;
            $get_kyc->sch_to_place_id   = null;
            
            if ($getlastJob) {
                // return $getLastJob;
                // $jsonData = json_decode($getlastJob->job_data, true);
                
                $get_kyc->sch_from_place = $getlastJob->from_place ?? null;
                $get_kyc->sch_to_place   = $getlastJob->to_place ?? null;
                $get_kyc->sch_from_place_id   = $getlastJob->from_place_id ?? null;
                $get_kyc->sch_to_place_id   = $getlastJob->to_place_id ?? null;
            ...
            if ($lastSpin) {
                ...
                if (isset($slotData['type']) && $slotData['type'] === 'free_spin') {
                    $can_spin = true; 
                } else {
                    $can_spin = false; 
                }
            ...
            if ($userFeedback) {
                if (!is_null($userFeedback->rating)) {
                    $feedback_status = true;
                } elseif (!is_null($userFeedback->dismissed_date)) {
                    $dismissedDateOnly = date('Y-m-d', strtotime($userFeedback->dismissed_date));
                    if ($dismissedDateOnly === $todayDate) {
                        $feedback_status = true;
                    }
                }
            }
            $get_kyc->feedback_status = $feedback_status;
            ...
            $get_kyc->rm_expiry = now()->addMonths(3)->format('Y-m-d');
            $get_kyc->popular_location = $pop_loc;
            $get_kyc->wallet_balance = auth()->user()->walletBalance;
            $get_kyc->address = auth()->user()->address;
            $get_kyc->per_km = auth()->user()->per_km??'2';
            $get_kyc->popup_time = auth()->user()->popup_time??null;
            $get_kyc->popup_show = auth()->user()->popup_show;
                ...
                DB::transaction(function () use ($userId, $lat, $lng, $now) {
                    ...
                    try {
                        ...
                        if (!empty($state) || !empty($district) || !empty($fullAddress)) {
                            ...
                            DB::table('drivers_current_location')
...
Step-by-step Partner App tutorials
            ...
            if($get_kyc_1 == null){
                ...
                $get_kyc = [
                    ...
                    "trac_id" => null,
                    "crm_status" => 0,
                    "can_spin" => $can_spin, // Added here as well
                    'how_to_use' => [
                        'bidding_job' => 'https://www.youtube.com/watch?v=_KNRIsVe-Yw',
                        'fixed_job' => 'https://www.youtube.com/watch?v=1SkQ-xmBzSY'
                    ],
...
Step-by-step Partner App tutorials",
           ...
           $validated = $request->validate([
            ...
            if ($userId != '') {
                if($request->type == 'AADHAAR'){
                    ...
                    $check_ocr = DB::connection('mysql_log')->table('ocr_request')->where(['user_id' => $userId, 'status' => 'approved', 'deletes' => 0])->exists();
                    ...
                    $id = DB::connection('mysql_log')->table('ocr_request')->insertGetId($ins_req);
                    ...
                    if ($response->getStatusCode() >= 200 && $response->getStatusCode() < 300) {
                        ...
                        DB::connection('mysql_log')->table('ocr_request')->where('id', $id)->update([
    ...
    public function doc_verify(Request $request)
    {
        try {
            ...
            if ($request->type == 'AADHAAR') {
                ...
                $check_ocr = DB::connection('mysql_log')->table('ocr_request')
                    ->where([
                        'user_id' => $userId,
                        'status'  => 'approved',
                ...
                // Insert request
                $id = DB::connection('mysql_log')->table('ocr_request')->insertGetId([
    ...
    try {
        ...
        if ($request->type == 'DRIVING_LICENSE') {
            ...
            DB::connection('mysql_log')->table('ocr_request')->insert([
