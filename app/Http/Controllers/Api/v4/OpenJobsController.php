...
class OpenJobsController extends Controller
{
    ...
    public function DistanceAndDuration(Request $request)
    {
        try {
            ...
            $cities_form = DB::table('districts')
                ->select('id', 'district_name', 'state', 'latitude', 'longitude')
                ->where('deletes', 0)
                // ->where(DB::raw("CONCAT(district_name, ', ', state)"), $request->from)
                ->where('name', $request->from)
                // ->where(['country_code' => 'IN', 'state_code' => 'TN']) // Remove state_code if 'state' column is used instead
                ->first();
            
            // Check "to" location
            $cities_to = DB::table('districts')
                ->select('id', 'district_name', 'state', 'latitude', 'longitude')
                ->where('deletes', 0)
                // ->where(DB::raw("CONCAT(district_name, ', ', state)"), $request->to)
                ->where('name', $request->to)
                // ->where(['country_code' => 'IN', 'state_code' => 'TN']) // Remove state_code if 'state' column is used instead
                ->first();
            ...
            $data = [];
            if ($request->pass && $request->pass <= 5) {
                $column = 'four_seater';
        ...
        try {
            ...
            if (auth()->user()->isPostJob == '0') {
                return response()->json([
                    ...
                    'message' => 'Job posting is currently disabled for your account. Please contact our support team to enable this service.',
                ], 200);
            }
        ...
        try {
            ...
            if (auth()->user()->isOpenjob == '0') {
                ...
                return response()->json([
                    ...
                    'message' => 'Job list access is currently disabled for your account. Please contact our support team to enable this service.',
                ], 200);
            }
            ...
            $firebaseDocs = Cache::remember('jobs_list_global', 5, function () {
                return DB::table('cus_job_temp as oj')
                    ...
                    ->where('oj.pickup_date', '>=', now())
                    // ->where('oj.user_id', '!=', 0)
                    ->whereNotIn('oj.global_type', ['carpool', 'dr_carpool'])
                    ->where(function ($q) {
                        $q->where('oj.global_type', 'customer')
                          ->orWhere('oj.confirm_status', 1);
                    })
            ...
            $firebaseJobs = collect($firebaseDocs)
                ...
                ->filter(function ($job) use (
                    ...
                    // Use stripos for case-insensitive matching to protect against "tamil nadu" vs "Tamil Nadu"
                    if (stripos($from_place, $u_state) === false) {
                        return false;
                    }
    ...
    public function my_current_jobs(Request $request)
    {
        ...
        $jobs = Cache::remember($cacheKey, 5, function () use ($userId) {
            ...
            return DB::table('cus_job_temp as oj')
                ...
                ->where('oj.pickup_date', '>=', Carbon::now())
                ->where('oj.user_id', $userId)
                ->whereIn('oj.global_type', ['open', 'mock', 'dr_carpool'])
                ->where('oj.deletes', '0')
                ->where('ur.deletes', '0')
                ->whereNotIn('oj.job_status', ['expiried', 'cancelled', 'no_response'])
                ->select(
    ...
    //                 $allBids[$job->job_no] = json_decode(
    //                     $job->bids_details,
    //                     true
    //                 );
    ...
    //                                 AND i.invitee_user_id = " . (int)$userId . "
    //                                 AND i.type = 'join'
    //                             THEN i.inviter_id
    ...
    //                     'i.invite_token',
    //                     'i.type as jb_type',
    //                     DB::raw('c.id as user_id'),
    //                     DB::raw('c.name as name'),
    //                     DB::raw('c.profile_img_url as profile_img_url'),
    //                     DB::raw('c.fcm_token as fcm_token')
    //                 )
    //                 ->get();
    ...
    //         $formattedJobs = $jobs->map(function ($job) use (
    //             $allBids,
    //             $users,
    //             $groupedInvites,
    //             $deep
        ...
        try {
            ...
            if (auth()->user()->isBidding == '0') {
                return response()->json([
                    ...
                    'message' => 'Job bidding is currently disabled for your account. Please contact our support team to enable this service.',
                ], 200);
            }
...
Thanks for using " . env('APP_NAME') . ".  ";
    ...
    public function bidding_status(Request $request)
    {
        try {
            ...
            $inStatus = ['accept', 'reject', 'direct', 'inreview', 'available', 'busy'];
            $jobStatusAllowed = ['accept', 'bidding', 'cancelled', 'created', 'schedule', 'started', 'completed'];
            ...
            $firebaseDocs = Cache::remember('bidding_job_global', 5, function () use ($jobStatusAllowed) {
                ...
                return DB::table('cus_job_temp as oj')
                    ...
                    ->whereIn('oj.job_status', $jobStatusAllowed)
                    ->where(function ($query) {
                        $query->where('oj.job_status', 'started')
                              ->orWhere('oj.created_at', '>=', now()->subDays(5));
                    })
                    // ->where('oj.pickup_date', '>=', now())
            ...
            // \Log::info('Bidding dataaaaaaaaaaa', $firebaseDocs->toArray());
            ...
            foreach ($firebaseDocs as $job) {
                ...
                if($job->global_type == 'schedule'){
                    ...
                    if($job->job_status == 'accept' || $job->job_status == 'started' || $job->job_status == 'completed'){
                ...
                if ($job->job_status == 'accept' &&
                    ...
                    in_array($userBid['status'], ['available'])) {
        
                    $userBid['status'] = 'accept';
        
                }
                
                if (($userBid['status'] ?? null) == 'inreview' && !empty($pickup)) {
                    if (Carbon::parse($pickup, 'Europe/London')->isPast()) {
                        continue;
                    }
                }
...
Your job **{$get_job->job_no}** has been ❌ *cancelled*.  
...
";
    ...
    public function cancelRide(Request $request)
    {
        ...
        try {
            ...
            if ($ch_payment && $customer && $job->job_status == 'accept') {
                ...
                if ($job->isView == 0 && !$alreadyRefunded) {
            ...
            if ($job->global_type != 'schedule' && $job->job_status != 'accept' && $job->job_status != 'started' && $job->job_status != 'completed') {
                ...
                if ($customer) {
                    dispatch(new \App\Jobs\SendFcmNotificationJob(
                        type: 'job_cancelled_driver',
                        userIds: [$customer->fcm_token],
                        title: 'Job Cancelled',
                        body: 'Job ' . $job->job_no . ' has been cancelled by the driver.'
                    ));
                }
        ...
        try {
            ...
            if($request->user_id != ''){
                ...
                if($get_id->type == 'Owner'){
                    ...
                    $get_rc = optional(
                        DB::connection('mysql_log')->table('ocr_request')
                    ...
                    $get_seat = DB::connection('mysql_log')->table('ocr_request')
    ...
    public function schedule_create(Request $request)
    {
        ...
        if (auth()->user()->isPostSchJob == '0') {
                return response()->json([
                    ...
                    'message' => 'Scheduled ride booking is currently disabled for your account. Please contact our support team to enable this service.',
                ], 200);
            }
...
";
        ...
        try {
            ...
            if ($get_fare) {
                ...
                if (is_array($dates_data)) {
                    ...
                    foreach ($dates_data as $dateTime => $amount) {
                
                        // Compare only date part
                        if (date('Y-m-d', strtotime($dateTime)) == $searchDate) {
                
                            $base_fare = (int) $amount;
                            break;
                        }
                    }
                }
    ...
    public function otpVerify(Request $request)
    {
        try {
            ...
            DB::transaction(function () use ($otp, $data, $user, $request) {
