    ...
    public function cancelJob(Request $request)
    {
        try {
            ...
            dispatch(function () use ($jobDataArr, $controller) {
                ...
                $userId = (int) ($get_job_obj->user_id ?? 0);
                ...
                if (empty($customerMobile) && !empty($get_job_obj->user_details)) {
                    $userDetails = is_string($get_job_obj->user_details) ? json_decode($get_job_obj->user_details, true) : (array) $get_job_obj->user_details;
                ...
                if (!empty($customerMobile)) {
                    ...
                    if (!empty($cleanPhone)) {
                        ...
                        $pickupLoc = !empty($get_job_obj->pick_address) ? $get_job_obj->pick_address : (!empty($get_job_obj->from_place) ? $get_job_obj->from_place : 'Not Specified');
                        $dropLoc   = !empty($get_job_obj->drop_address) ? $get_job_obj->drop_address : (!empty($get_job_obj->to_place) ? $get_job_obj->to_place : 'Not Specified');
