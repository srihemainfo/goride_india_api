...
class CustomerAppController extends Controller
{
    ...
    public function adminAutoCompleted(Request $request)
    {
        try {
            ...
            if ($bearerToken !== env('EXPECTED_API_TOKEN')) {
                return response()->json([
                    'success' => false,
    ...
    public function GoogleLocations(Request $request)
    {
        try {
            ...
            // ✅ No state restriction (All India)
            $query = DB::table('outstation_locations as ol')
                ->where('ol.search_key', 'LIKE', $searchKey . '%');
    
            $sub = DB::table('outstation_locations')
            ...
            $dbResults = $query
                ...
                ->whereRaw("(LENGTH(display_name) - LENGTH(REPLACE(display_name, ',', ''))) >= 1")
                ->orderByRaw("(LENGTH(display_name) - LENGTH(REPLACE(display_name, ',', ''))) ASC")
                ->limit(10)
                ->get([
                    'ol.display_name as name',
                    'ol.place_id',
                    'ol.latitude',
                    'ol.longitude'
                ])
            ...
            if (($google['status'] ?? '') !== 'OK') {
                ...
                Cache::store('redis')->put($cacheKey, $dbResults, now()->addMinutes(30));
            ...
            foreach ($google['predictions'] as $p) {
                ...
                foreach ($parts as $part) {
                    ...
                    // ✅ First meaningful part = district
                    if (!$district) {
                        $district = $part;
                        continue;
                    }
    
                    // ✅ Second meaningful part = state
                    if (!$state) {
                        $state = $part;
                    }
                }
                ...
                // ✅ OPTIONAL: enforce district mandatory
...
public function AdminDistanceAndDurationAll(Request $request)
{
    try {
        ...
        // --- CRITICAL FIX: Add Geocoding Fallback for Plain Text Addresses ---
        $fromGeo = $this->getLatLngByPlaceId($request->from_place_id);
        if (!$fromGeo && preg_match('/[,\s]/', $request->from_place_id)) {
            $fromGeo = $this->fallbackGeocodeAddress($request->from_place_id);
        }

        $toGeo = $this->getLatLngByPlaceId($request->to_place_id);
        if (!$toGeo && preg_match('/[,\s]/', $request->to_place_id)) {
            $toGeo = $this->fallbackGeocodeAddress($request->to_place_id);
        }
        // ---------------------------------------------------------------------
        ...
        if (!$fromGeo || !$toGeo) {
            return response()->json([
                'status'  => false,
                'message' => 'Unable to resolve coordinates. Please re-select the locations from the dropdown.'
            ], 422);
        }
...
/**
 * HELPER METHOD: Geocode text addresses if Google Place ID is missing.
 * Add this method inside the same Controller class.
 */
private function fallbackGeocodeAddress($address)
{
    // Try to grab your active Google Map API Key
    $apiKey = env('GOOGLE_MAP_KEY') ?? env('GOOGLE_MAPS_API_KEY') ?? env('MAP_API_KEY');
    ...
    try {
        ...
        if ($data['isDriver'] == 'no') {
            ...
            if ($check_data && !empty($data['pass_count'])) {
                ...
                if ($settings && $settings->default_discount > 0) {
                    $discountAmount = ($base_fare * $settings->default_discount) / 100;
