    ...
    public function send(Request $request)
    {
        ...
        try {
            ...
            $job = DB::table('cus_job_temp')
                ...
                ->whereNot('user_id', $request->invitee_user_id)
                ->where('deletes', '0')
                ->first();
    
            if (!$job) {
                return ApiResponse::error('Cannot invite for this job', 403);
            }
    ...
    public function inviteList()
    {
        try {
            ...
            $invites = DB::table('invitations as i')
                ...
                // ✅ Only pending (latest state must be pending)
                ->where('i.status', 'pending')
    
                // ✅ CRITICAL FIX: If ANY accepted/rejected/removed exists → exclude job
