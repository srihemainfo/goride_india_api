  ...
  public function getPlanDetails($razorpay, $cart)
  {
    try {
      ...
      if (empty($filteredPlans)) {
        ...
        $notes = array_slice($cart['productDetails'], 0, 15, true);
  ...
  public function cancelSubCRM(Request $request)
  {
    ...
    try {
      ...
      if (($subscriptionL->gateway === 'razorpay' && isset($getSubscription) && isset($getSubscription->id) && $getSubscription->status == 'cancelled') || ($subscriptionL->gateway === 'paypal' && isset($getSubscription['status']) && $getSubscription['status'] == 'CANCELLED')) {
        ...
        $findDays = ($subscriptionL->planType === 'YEARLY' ? Controller::getDaysInMonthOrYear('year', $checkout_response['productDetails']['validityMorY'] * $paid_count, $startDate) : Controller::getDaysInMonthOrYear('month', $checkout_response['productDetails']['validityMorY'] * $paid_count, $startDate));



        $expiryDate = Carbon::parse($startDate)->addDays($findDays)->format('Y-m-d');
    ...
    public function cancelSubPlan(Request $request)
  {
    ...
    try {
      ...
      if($access_token != 'asdfghjklpoiuytrewqzxcvbnm1234567890)(*&^%$#@!'){
          $response = ['status' => 'failed', 'message' => 'Un Authorised', 'error' => 'Kindly check the access token!'];
            goto returnFVI;
      }
      // PayPal API credentials
      $clientId = config('paypal.client_id');
      ...
      $crmID = $request['crmID'];
    // return response()->json([$user_id, $crmID]);
      // $domainPrefix = $request->domainPrefix;
      // $userName = $request->userName;
      // $passWord = $request->passWord;
