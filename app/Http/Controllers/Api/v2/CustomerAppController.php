...
class CustomerAppController extends Controller
{
    ...
    public function GoogleLocationsOld(Request $request)
    {
        try {
    ...
    public function GoogleLocationsOld2(Request $request)
    {
        try {
            ...
            foreach ($google['predictions'] as $p) {
                ...
                if (!$mainText || empty($terms)) continue;
            
                $count = count($terms);
            
                $country  = $count >= 1 ? $terms[$count - 1]['value'] : null;
                $state    = $count >= 2 ? $terms[$count - 2]['value'] : null;
                $district = $count >= 3 ? $terms[$count - 3]['value'] : null;
            
                // ✅ Must be India
                if (strtolower($country) !== 'india') continue;
                
                // ✅ Validate state exists
                if (!$state) continue;
                
                // ❌ Remove fake district (if it's actually a state)
                if ($district && in_array($district, $state)) {
                    $district = null;
                }
                
                // ❌ Remove duplicate like "Kerala, Kerala"
...
— *GoRide Run Pvt Ltd*";
    ...
    public function paymentVerify(Request $request)
    {
        ...
        try {
            ...
            $pickupTime = $get_log->pickup_date ? date('d M Y, h:i A', strtotime($get_log->pickup_date)) : '-';
            ...
            $tripType = ucwords($get_log->job_type ?? '-');
            $fare = $total_fare ?? 0;
            $pay_to_driver = $fareBreakdown['pay_to_driver'] ?? 0;
...
🕒 *Pickup Time:* {$pickupTime}
💼 *Trip Type:* {$tripType}
💰 *Fare Details:* ₹{$pay_to_driver} (Payment Mode: Online)
