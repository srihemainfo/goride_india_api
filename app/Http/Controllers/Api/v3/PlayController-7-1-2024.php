...
class PlayController extends Controller
{
  ...
  public function generatePlanData($plan, $priceKey, $validityKey, $bookingsKey)
  {
    return [
      ...
      "price" => $plan->$priceKey,
      "validityDays" => $plan->$validityKey,
      "productType" => $plan->productType,
      "description" => $plan->description,
      "standardFeatures" => $plan->standardFeatures,
      "perDay" => ($plan->$priceKey > 0) ? number_format(($plan->$priceKey / $plan->$validityKey), 2) : 0
    ];
  }
  ...
  public function dashboard(Request $request)
  {
    ...
    ->select('c.*', 's.expiryDate', 's.checkout_response', 's.sub_status as subStatus')
 ...
 $item->checkout_response = json_decode($item->checkout_response, true);
  ...
  public function packageHistory(Request $request)
  {
    try {
      ...
      $transactions = DB::table('subscriptions as p')
