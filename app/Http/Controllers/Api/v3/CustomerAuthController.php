        ...
        if($customer){
            ...
            if ($hostCount > 0) {
                ...
                $upcomingJob = DB::table('cus_job_temp as ct')
                    // ->leftJoin('invitations as in', 'in.job_id', '=', 'ct.id')
                    // ->where(function ($query) use ($uid) { 
                    //     $query->whereNot(function ($subQuery) use ($uid) {
                    //         $subQuery->where('in.inviter_id', $uid)
                    //                  ->where('in.type', 'join');
                    //     })
                    //     ->orWhere(function ($subQuery) use ($uid) {
                    //         $subQuery->whereNot(function ($deepQuery) use ($uid) {
                    //             $deepQuery->where('in.invitee_user_id', $uid)
                    //                       ->where('in.type', 'job');
                    //         });
                    //     });
                    // })
                    ->leftJoin('invitations as in', function ($join) use ($uid) {
                        $join->on('in.job_id', '=', 'ct.id')
                             ->where(function ($query) use ($uid) {
                                 $query->whereNot(function ($subQuery) use ($uid) {
                                     $subQuery->where('in.inviter_id', $uid)
                                              ->where('in.type', 'join');
                            ...
                            //  ->whereNotIn('in.status', ['pending', 'accepted']);
                    })
        ...
        try {
            ...
            if ($user) {
                ...
                $data['user_details'] = [
                    ...
                    'isCarpool'       => auth()->user()->current_state == 'Tamil Nadu' ? 'no' : 'yes',
                    'cancel_reason' => [
