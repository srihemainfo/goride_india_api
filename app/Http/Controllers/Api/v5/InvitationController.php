...
class InvitationController extends Controller
{
    ...
    public function sync(Request $request)
    {
        try {
            ...
            DB::statement("
                ...
                WHERE uc.user_id = ? AND u.deletes = '0'
            ", [$userId]);
    ...
    public function send(Request $request)
    {
        ...
        try {
            ...
            if ($request->from_place_id != $getJob->from_place_id || $job->to_place_id != $job->to_place_id) {
                $isVal = true;
            }
            ...
            if ($getJob->global_type) {
                ...
                if($job->global_type == 'dr_carpool'){
                    ...
                    $cus = DB::table('customer_register')->where('id', $request->invitee_user_id)->first();
                    
                    $vehicleDetails = ($cus && $cus->vehicle_details) ? json_decode($cus->vehicle_details) : null;
                    
                    $isCar = ($vehicleDetails?->choosed_vehicle == 'CAR');
                    
                    $perKm = $isCar ? $get_set->carpool_car_price_km : $get_set->carpool_bike_price_km;
            ...
            $inviteId = DB::table('invitations')->insertGetId([
                ...
                'from_place_id' => $job->from_place_id??null,
                'to_place_id' => $job->to_place_id??null,
                'source' => 'contact_sync',
                'collectAmt' => $totFare,
                'deductAmt' => $dedAmt,
    ...
    public function joinRequest(Request $request)
    {
        ...
        try {
            ...
            \Log::info('Request payloads'. json_encode($request->all()));
    ...
    public function notificationList(Request $request)
    {
        try {
            ...
            */
            ...
            $groupInvites = DB::table('group_invitations as gi')
                ->join('society_groups as sg', 'sg.id', '=', 'gi.group_id')
                ->join('customer_register as c', 'c.id', '=', 'gi.invited_by')
                ->where('gi.invitee_user_id', $userId)
                ->where('gi.status', 'pending')
                ->select(
            
                    'gi.id',
            
                    DB::raw("'group_invitation' as event"),
            
                    'gi.status',
            
                    'gi.created_at as sent_at',
            
                    DB::raw("'👥 Society Group Invitation' as title"),
            
                    DB::raw("
                        CONCAT(c.name,' invited you to join ',sg.name,'.')
                        as message
                    "),
            
                    DB::raw("'group-invitation' as redirect"),
            
                    DB::raw("'society' as type")
            ...
            $societyJobs = DB::table('group_job_shares as gjs')
            
                ->join('society_groups as sg', 'sg.id', '=', 'gjs.group_id')
            
                ->join('customer_register as c', 'c.id', '=', 'gjs.shared_by')
            
                ->whereIn('gjs.group_id', $groupIds)
            
                ->where('gjs.status', 'active')
            
                ->where('gjs.shared_by', '!=', $userId)
            
                ->select(
            
                    'gjs.group_id as id',
            
                    DB::raw("'society_job' as event"),
            
                    DB::raw("'active' as status"),
            
                    'gjs.created_at as sent_at',
            
                    DB::raw("'🚖 New Society Ride' as title"),
            
                    DB::raw("
                        CONCAT(c.name,' shared a ride in ',sg.name,'.')
                        as message
                    "),
            
                    DB::raw("'society-jobs' as redirect"),
