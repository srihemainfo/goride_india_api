...
class CustomerAppController extends Controller
{
    ...
    public function getNearbyFcm($fromLat, $fromLng, $limit = 50, $cab_seat = null, $state = null)
    {
        ...
        $chCount = DB::table('user_register as ur')
            ->join('drivers_current_location as dcl', 'ur.id', '=', 'dcl.user_id')
            ->whereNotNull('ur.state')
            ->whereNotNull('ur.fcm_token')
            ->where('ur.deletes', '0')
            ->where('ur.doc_verify', 1)
            ->where('ur.vehicle_verify', 2)
            ->where(function ($query) use ($state) {
                $query->where('dcl.current_state', $state)
                      ->orWhere('ur.state', $state);
            })
            ->where('ur.notify', 1)
            ->where('dcl.updated_at', '<=', now()->subDays(1))
            ->count();
        
        if ($chCount < 100 && env('APP_ENV') != 'local') {
        // if ($chCount < 100) {
            return collect();
        }
        ...
        foreach ($radiusSteps as $radius) {
            ...
            $drivers = DB::table('user_register as ur')
                ...
                ->whereNotIn('ur.id', $collected->pluck('id')->toArray())
    
                ->whereNotNull('dcl.lat')
                ->whereNotNull('dcl.lng')
    
                ->whereBetween('dcl.lat', [
                    $fromLat - $latRange,
                    $fromLat + $latRange
                ])
    
                ->whereBetween('dcl.lng', [
                    $fromLng - $lngRange,
                    $fromLng + $lngRange
                ])
    ...
    private function createFirebaseJob(string $jobNo, array $data)
    {
        ...
        $fbCol = env('FIREBASE_COLLECTION', 'dev_jobs');
        ...
        $fields = [
                ...
                // --- ADDED NOTIFICATION COUNT KEYS ---
                'total_notifications' => [
                    'integerValue' => (string) (int) ($data['total_notifications'] ?? 0)
                ],
                'success_count' => [
                    'integerValue' => (string) (int) ($data['success_count'] ?? 0)
                ],
                'failure_count' => [
                    'integerValue' => (string) (int) ($data['failure_count'] ?? 0)
                ],
                // -------------------------------------
    ...
    private function getRouteData($fromLat, $fromLng, $toLat, $toLng)
    {
        ...
        $gKeys = array_values(array_filter([
            env('DISTANCE_GOOGLE_KEY'),
            env('DISTANCE_GOOGLE_KEY_TWO'),
            env('DISTANCE_GOOGLE_KEY_THREE'),
        ]));
            ...
            if ($response->successful()) {
                ...
                if (isset($data['routes'][0])) {
                    ...
                    return [
                        ...
                        // 'duration'    => gmdate("H:i:s", $route['duration']),
    ...
    public function GoogleLocationsLastOld(Request $request)
    {
        try {
            $search = trim($request->search);
    ...
    public function GoogleLocationsOld_4_7_26(Request $request)
    {
        try {
    ...
    public function GoogleLocations(Request $request)
    {
        try {
            $search = trim($request->search);
            $method = trim(strtolower($request->method ?? 'outstation'));
            $s_type = trim(strtolower($request->search_type ?? 'from_location'));
            ...
            // Dynamic State Constraint Logic
            $allowedStates = [];
            if ($method !== 'carpool') {
                // Outstation restrictions apply
                if ($s_type == 'from_location') {
                    $allowedStates = ['Tamil Nadu'];
                } else {
                    $allowedStates = ['Tamil Nadu', 'Kerala', 'Puducherry', 'Pondicherry', 'Andhra Pradesh', 'Karnataka'];
                }
            }
            // If method is carpool, $allowedStates remains empty, opening it up to all of India.
    
            $table = $method == 'carpool' ? 'carpool_locations' : 'outstation_locations';
            $limit = 5;
    
            $searchKey = $this->normalizeSearch($search);
            $cacheKey = "location_search:" . $method . '_' . $s_type . ":" . $searchKey;
            ...
            // Apply state restrictions only if outstation rules dictate it
            if (!empty($allowedStates)) {
                $dbQuery->whereIn('state', $allowedStates);
            }
    
            $dbResults = $dbQuery->orderByRaw("
                    CASE
                        WHEN search_key LIKE ? THEN 1
                        ELSE 2
                    END
            ...
            // Formulate regional constraints (Country is always India)
            $components = 'country:in';
            // if (!empty($allowedStates)) {
             ...
             * Outstation: '(regions)' focuses heavily on cities, towns, and districts.
             * Carpool: Empty allows streets, checkpoints, specific interior structures, and landmarks.
             */
            $googleTypes = ($method === 'carpool') ? '' : '(regions)';
            ...
            if (!empty($googleTypes)) {
                $googleParams['types'] = $googleTypes;
            }
    
            $google = Http::timeout(3)
            ...
            foreach ($google['predictions'] as $p) {
                ...
                // This is the full address string (e.g., "Taj Mansingh Hotel Road, South Block, Man Singh Rd...")
                $fullAddress = trim($p['description'] ?? '');
                $placeId = trim($p['place_id'] ?? '');
            
                if (!$fullAddress || !$placeId) {
                    continue;
                }
            
                // FIX: Pull the clean short name from structured_formatting (e.g., "Taj Mansingh Hotel Road")
                // Fall back to full address only if main_text is missing
                $shortName = trim($p['structured_formatting']['main_text'] ?? $fullAddress);
                ...
                // Secondary state verification rule fallback (Skip if carpool allows all India)
                if (!empty($allowedStates) && !in_array($state, $allowedStates)) {
                    continue;
                }
                ...
                $exists = collect($newResults)
                    ...
                    ->contains(function ($x) use ($placeId, $formattedName) {
                        ...
                        return $id == strtolower($placeId) || $name == strtolower($formattedName);
                    });
                ...
                // Send short name to user interface array matching your DB conversion schema
                $newResults[] = [
                    'name'      => $formattedName, // Keeps full description for autocomplete dropdown UI
                    'id'        => $placeId,
                    'latitude'  => $lat,
                    'longitude' => $lng,
                    'state'     => $state,
                ...
                // FIX: Map fields cleanly for database insertion
                $insertData[] = [
                    'place_id'     => $placeId,
                    'name'         => $shortName,     // Stored cleanly as "Taj Mansingh Hotel Road"
                    'display_name' => $fullAddress,   // Stored as full text description
                    'state'        => $state,
                    'district'     => $district,
                    'country'      => 'India',
                    'latitude'     => $lat,
                    'longitude'    => $lng,
                    'search_key'   => $this->normalizeSearch($fullAddress),
                    'source'       => 'google',
                    'res_json'     => json_encode($p),
                    'created_at'   => now(),
                    'updated_at'   => now(),
            ...
            if (!empty($insertData)) {
                DB::table($table)->upsert(
                    $insertData,
                    ['place_id'],
                    ['name', 'display_name', 'state', 'district', 'latitude', 'longitude', 'updated_at']
                );
            }
    
            $final = collect(array_merge($dbResults, $newResults))
                ->unique(function ($item) {
                    return strtolower(trim(($item['id'] ?? '') . '|' . ($item['name'] ?? '')));
                })
                ->values()
                ->take($limit)
                ->toArray();
            ...
            if (!empty($final)) {
                Cache::put($cacheKey, $final, now()->addMinutes(2));
            }
    ...
    public function DistanceAndDuration(Request $request)
    {
        try {
            ...
            if ($pickup->isToday() && $pickup->lessThanOrEqualTo($now->copy()->addMinutes(30))) {
                return response()->json([
                    'status'  => false,
...
//                     $data['total_notifications'] = isset($fcmTokens) ? count($fcmTokens) : 0;
...
//                 $get_lat = DB::table('outstation_locations')->where('place_id', $data['from_place_id'])->select('latitude', 'longitude', 'state')->first();
                
//                 $fcmTokens = [];
                
//                 if($get_lat){
                    
//                     $parts = array_map('trim', explode(',', $data['from_place']));
                    
//                     $stateee = null;

//                     foreach (array_reverse($parts) as $part) {
                    
//                         $state = preg_replace('/\b\d{6}\b/', '', $part);
//                         $state = trim($state);
                    
//                         if (
//                             $state &&
//                             strtolower($state) !== 'india' &&
//                             !preg_match('/^\d+$/', $state)
//                         ) {
//                             $stateee = $state;
//                             break;
...
//                     $data['total_notifications'] = count($fcmTokens);
//                     $data['success_count'] = 0;
//                     $data['failure_count'] = 0;
        ...
        try {
            ...
            if ($samePickupJobs->isNotEmpty()) {
                ...
                return response()->json([
                    ...
                    'message' => 'A booking already exists for the selected pickup date. Please choose another date.'
                ], 422);
            }
...
*GoRide System*";
            ...
            if ($data['isDriver'] == 'no') 
            {
                ...
                if ($create_job) {
                    ...
                    if (!empty($fcmTokens)) {
                        $job = new \App\Jobs\SendJobNotificationJob(
                            [
                                'id'      => $create_job,
                                'job_no'  => $job_no,
                                'type'    => 'new_job_notification',
...
*GoRide System*";
    ...
    public function createOrder(Request $request)
    {
        ...
        if($get_data->bids_details){
            ...
            $bids['amount'] = (int)$bids['amount'];
            $fr_details['actual_driver_amt'] = (int)$fr_details['actual_driver_amt'];
            
            if($bids['amount'] != $fr_details['actual_driver_amt']){
                
                return response()->json([
                    'status' => false,
                    'message' => 'The trip fare has changed. Please refresh to continue with your payment.'
                ]);
            }
        ...
        if($request->status && in_array($request->status, ['created', 'accept', 'cancelled'])){
            ...
            $book_history = DB::table('cus_job_temp as ct')
                ...
                ->where(function ($q) use ($now) {
                    $q->whereIn('ct.job_status', ['accept','cancelled', 'started', 'completed'])
                      ->orWhere('ct.pickup_date', '>=', $now);
                })
                ->where(function ($q) {
                    $q->where(function ($subQ) {
                        // Group 1: Cancelled records within the last 2 hours
                        $subQ->where('ct.job_status', '=', 'cancelled')
                             ->where('ct.pickup_date', '>=', Carbon::now()->subHours(2));
                    })
                    ->orWhere(function ($subQ) {
                        // Group 2: Completed records within the last 2 days
                        $subQ->where('ct.job_status', '=', 'completed')
                             ->where('ct.pickup_date', '>=', Carbon::now()->subDays(2));
                    })
                    ->orWhere(function ($subQ) {
                        // Group 3: All other statuses within the last 9 days
                        $subQ->whereNotIn('ct.job_status', ['cancelled', 'completed'])
                             ->where('ct.created_at', '>=', Carbon::now()->subDays(9)->startOfDay());
                    });
                })
                ...
                ->select([
                    ...
                    DB::raw("
                        CASE
                            WHEN (ct.job_status = 'accept' OR ct.job_status = 'started') AND ct.pickup_date >= '$now'
                            THEN 'confirmed'
                            ELSE ct.job_status
                        END AS job_status
                    "),
                    ...
                    DB::raw("
                        CASE
                            WHEN ct.job_status = 'cancelled' OR ct.job_status = 'completed' OR ct.job_status = 'started'
                            THEN 'no'
                            ELSE 'yes'
                        END as isOtp
                    ")
    ...
    public function savedPlaceStore(Request $request)
    {
        $validator = Validator::make($request->all(), [
        ...
        /*
        Check existing saved place
        Example:
        One Home per user
        */

        $existingPlace = DB::table('saved_places')
        ...
        if ($existingPlace) {
            ...
            $updatedPlace = DB::table('saved_places')
                ->where('id', $existingPlace->id)
                ->first();
        ...
        $id = DB::table('saved_places')->insertGetId([
        ...
        $savedPlace = DB::table('saved_places')
            ->where('id', $id)
            ->first();
    ...
    public function savedPlaceList(Request $request)
    {
        if($request->type){
    ...
    public function savedPlaceEdit(Request $request, $id)
    {
        $validator = Validator::make($request->all(), [
            'type' => 'required|string|max:255',
            'address' => 'required|string',
    ...
    public function savedPlaceDelete($id)
    {
        $savedPlace = DB::table('saved_places')
            ->where('id', $id)
            ->where('user_id', auth()->id())
    ...
    public function fetchAddress(Request $request)
    {
        ...
        try {
            ...
            if (!$response->successful()) {
                return response()->json([
                    'status' => false,
                    'error'  => 'Failed to connect with Google API'
                ], 500);
            }
            ...
            return response()->json([
                ...
                'data'   => [
                    ...
                    'lat'          => round($result['geometry']['location']['lat'] ?? $lat, 6),
                    'lng'          => round($result['geometry']['location']['lng'] ?? $lng, 6),
