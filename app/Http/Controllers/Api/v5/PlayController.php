  ...
  public function dashboard(Request $request)
  {
    ...
    try {
      ...
      $crmList = DB::table('crm as c')
        ...
        ->leftJoin('user_register as ur', 'ur.id', '=', 'c.userID')
        ->select('c.*', 's.expiryDate', 's.checkout_response',  's.subscription_id as subscrID', 's.sub_status as subStatus', 's.subID', 's.subRes')
        ->where('c.userID', auth()->user()->id)
        ->where('c.deletes', '0')
        // ->whereNot('s.paymentStatus', 'cancelled')
        ->orderBy('c.id', 'desc')
        ->get()
        ->map(function ($item) {
        ...
        $item->isActive =  Carbon::parse( $item->expiryDate)->isAfter(Carbon::now());
          ...
          $item->subRes = isset($item->subRes) ? json_decode($item->subRes, true) : $item->subRes;
      ...
      $response = ['status' => 'failed', 'message' => 'Process failed Try After Some Time', 'error' => ['message' => $e->getMessage(), 'code' => $e->getCode(), 'string' => $e->__toString()]];
      return response()->json($response);
    }
  }
...
public function checkDomainStatus(Request $request)
  {
    ...
    $data['crmDetails'] = null;
      $data['subDetails'] = null;
...
//  $user_id = auth()->user()->id;
  ...
  $user_id=  $checkDomain->userID;
    ...
    ->select(
        ...
        'i.id as invID',
        DB::raw("CASE 
                    WHEN p.paymentStatus IN ('COMPLETED', 'paid', 'SUCCESS', 'active') THEN 'Active'
    ...
    ->where('p.user_id', '=', $user_id)
     ...
     ->where('p.id', '=', $checkDomain->subscription_id)
...
if( $invoice->count() > 0){
 ...
 if ($getSubsCrbtion->count() < 1) {
      $response = ['status' => 'failed', 'message' => 'Subscription track not found!', 'error' => 'Subscription track not found!'];

        goto returnFVI;
      }
      
      $getSubsCrbtion = $getSubsCrbtion[0];
      
      
    //   dd($getSubsCrbtion);

 
$data['subDetails'] = $getSubsCrbtion;
      ...
      $response = ['status' => 'success', 'message' => 'The crm details has been found successfully', 'data' => $data];
      goto returnFVI;
  ...
  public function generateCRM(Request $request)
  {
    ...
    try {
      ...
      if ($ticketUpdate) {
        ...
        $crmUrl = env('SHICreateCRM');
        
        // // If you want to disable SSL verification (for self-signed certs)
        // if (str_starts_with($crmUrl, 'https://')) {
        //     // HTTPS + self-signed: ignore SSL issues
        //     $createCRM = Http::withoutVerifying()->post($crmUrl, $reqArr);
        // } else {
        //     // HTTP (no SSL involved)
        // }
        $createCRM = Http::post($crmUrl, $reqArr);
        ...
        if ($createCRM->successful() && isset($crmRes['status']) && $crmRes['status'] === 'success') {
          ...
          if ($crmUpdate) {
            // dd('hii', $createCRM);
            $response = ['status' => 'success', 'message' => 'Your CRM is set up. Click the URL and enter your username and password.', 'data' => $crmRes];
            goto returnFVI;
          } else {
            $response = ['status' => 'failed', 'message' => 'Your CRM Setup Update Process Failed!', 'error' => 'The CRM Update Process failed!'];
            goto returnFVI;
          }
        } else {
            // dd('hello', $createCRM);
          $response = ['status' => 'failed', 'message' => 'Your CRM Generate Process Failed!', 'error' => 'The CRM generate process failed!', 'resErr' => $crmRes];
          goto returnFVI;
        }
      } else {
        $response = ['status' => 'failed', 'message' => 'Your CRM Setup Update Process Failed!', 'error' => 'The CRM Update process failed!'];
  ...
  public function generateCRM_mobile(Request $request)
  {
    ...
    try {
      ...
      if ($ticketUpdate) {
        ...
        GenerateCrmJob::dispatch($crmID, $reqArr, $domainPrefix, $user_id, $request->getHost())->onQueue('crm-creation');
        ...
        if (true) {
            ...
            $response = ['status' => 'success', 'message' => 'Your CRM is set up. Click the URL and enter your username and password.', 'data' => []];
            goto returnFVI;
...
if( $invoice->count() > 0){
  ...
  public function crmActivation(Request $request){
    //   dd($request);
    try {
        ...
        $crmQuery = DB::table('crm')
            ->where('id', $request->id)
            ->where('subscription_id', $request->subId);
        
        $updated = $crmQuery->update(['active_status' => $request->status]);
    
        if ($updated) {
            $fullDomain = $crmQuery->value('fullDomain');
        ...
        return response()->json([
            'status' => 'error',
            'message' => 'CRM status updation failed',
            'data' => []
        ]);
